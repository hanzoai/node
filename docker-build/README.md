...
```sh
DOCKER_BUILDKIT=1 docker build -f Dockerfile-RELEASE -t hanzoai/hanzo-node:latest ../
```
...
[INFO]  Docker building hanzoai/hanzo-node:latest using Dockerfile-RELEASE with source at ../
...
[INFO]  Docker save hanzoai/hanzo-node:latest to hanzoai_hanzo-node.tar
...
hanzo_deploy/hanzoai_hanzo-node.tar
```
...
[INFO]  Docker loading hanzoai_hanzo-node.tar
Loaded image: hanzoai/hanzo-node:latest
[INFO]  Edit ".env" if you want to start the node with preconfigured ai agents. You can also add ai agents from Hanzo Desktop.
[INFO]  Once done with ".env" changes, to start on-prem infrastructure run: docker compose up -d
[INFO]  Once everything is up and running, install/start Hanzo Desktop and use the default provided settings on the ui.
```
