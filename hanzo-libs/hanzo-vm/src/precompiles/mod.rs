...
//! | `0x0300..0000`    | AI mining (cross-chain compatible)   |
...
pub use ai_mining::ADDR_AI_MINING;
...
impl Default for PrecompileRegistry {
    ...
    fn default() -> Self {
        ...
        // AI Mining precompile (0x0300...) — shared address across Lux/Hanzo/Zoo
        // so a Go-produced proof is accepted by the Rust EVM and vice versa.
        ai_mining::register_ai_mining(&mut r);
...
// ---------------------------------------------------------------------------
// Tests for the built-in precompiles
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    #[test]
    fn default_registry_has_all_precompiles() {
        let reg = PrecompileRegistry::default();
        // 4 original + 1 AI mining
        assert_eq!(reg.len(), 5);
        ...
        assert!(reg.get(&ADDR_AI_MINING).is_some());
    }
