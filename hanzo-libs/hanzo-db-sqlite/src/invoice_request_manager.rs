use rusqlite::params;
use hanzo_messages::schemas::{invoices::InternalInvoiceRequest, hanzo_name::HanzoName};
...
impl SqliteManager {
    ...
    pub fn get_internal_invoice_request(&self, unique_id: &str) -> Result<InternalInvoiceRequest, SqliteManagerError> {
        ...
        Ok(InternalInvoiceRequest {
            ...
            provider_name: HanzoName::new(row.get(0)?).map_err(|e| {
                rusqlite::Error::ToSqlConversionFailure(Box::new(SqliteManagerError::SerializationError(e.to_string())))
            })?,
            requester_name: HanzoName::new(row.get(1)?).map_err(|e| {
                rusqlite::Error::ToSqlConversionFailure(Box::new(SqliteManagerError::SerializationError(e.to_string())))
            })?,
...
mod tests {
    ...
    use hanzo_messages::schemas::{hanzo_name::HanzoName, hanzo_tool_offering::UsageTypeInquiry};
