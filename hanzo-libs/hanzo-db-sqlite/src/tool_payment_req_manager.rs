use rusqlite::params;
use hanzo_messages::schemas::hanzo_tool_offering::{HanzoToolOffering, UsageType};

use crate::{SqliteManager, SqliteManagerError};

impl SqliteManager {
    pub fn set_tool_offering(&self, tool_offering: HanzoToolOffering) -> Result<(), SqliteManagerError> {
    ...
    pub fn get_tool_offering(&self, tool_key: &str) -> Result<HanzoToolOffering, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn
    ...
    pub fn get_all_tool_offerings(&self) -> Result<Vec<HanzoToolOffering>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare(
...
mod tests {
    ...
    async fn test_set_and_get_tool_offerings() {
        ...
        let tool_offering = HanzoToolOffering {
            tool_key: "tool_key".to_string(),
    ...
    async fn test_upsert_tool_offering() {
        ...
        let updated_tool_offering = HanzoToolOffering {
            tool_key: "tool_key".to_string(),
            usage_type: UsageType::PerUse(ToolPrice::Payment(vec![PaymentRequirements {
                scheme: "exact".to_string(),
    ...
    async fn test_remove_tool_offering_with_invoices() {
        ...
        use hanzo_messages::schemas::wallet_mixed::PublicAddress;
        ...
        // Verify that the invoice still exists but with hanzo_offering_key set to NULL
        ...
        let mut stmt = conn
            .prepare("SELECT invoice_id, hanzo_offering_key FROM invoices WHERE invoice_id = ?1")
            .unwrap();
        ...
        match result {
            Ok((invoice_id, offering_key)) => {
                ...
                assert!(
                    ...
                    "hanzo_offering_key should be NULL after tool offering removal"
                );
                println!("Verified: hanzo_offering_key is NULL as expected");
            }
            Err(err) => {
