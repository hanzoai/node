...
impl SqliteManager {
    pub fn set_invoice(&self, invoice: &Invoice) -> Result<(), SqliteManagerError> {
        ...
        match self.get_tool_offering(&invoice.hanzo_offering.tool_key) {
            Err(SqliteManagerError::ToolOfferingNotFound(_)) => {
                self.set_tool_offering(invoice.hanzo_offering.clone())?
            }
            Err(e) => return Err(e),
            Ok(_) => {}
        }
        ...
        stmt.execute(params![
            ...
            invoice.hanzo_offering.tool_key,
    ...
    pub fn get_invoice(&self, invoice_id: &str) -> Result<Invoice, SqliteManagerError> {
        ...
        let invoice = stmt
            .query_row(params![invoice_id], |row| {
                ...
                let hanzo_offering_key: String = row.get(4)?;
                ...
                Ok(Invoice {
                    ...
                    parent_message_id,
                    provider_name: HanzoName::new(provider_name).map_err(|e| {
                        rusqlite::Error::ToSqlConversionFailure(Box::new(SqliteManagerError::SerializationError(
                            e.to_string(),
                        )))
                    })?,
                    requester_name: HanzoName::new(requester_name).map_err(|e| {
                        rusqlite::Error::ToSqlConversionFailure(Box::new(SqliteManagerError::SerializationError(
                            e.to_string(),
                        )))
                    })?,
                    ...
                    hanzo_offering: self
                        .get_tool_offering(&hanzo_offering_key)
        ...
        let error = stmt
            .query_row(params![invoice_id], |row| {
                Ok(InvoiceRequestNetworkError {
                    invoice_id: row.get(0)?,
                    provider_name: HanzoName::new(row.get(1)?).map_err(|e| {
                        rusqlite::Error::ToSqlConversionFailure(Box::new(SqliteManagerError::SerializationError(
                            e.to_string(),
                        )))
                    })?,
                    requester_name: HanzoName::new(row.get(2)?).map_err(|e| {
                        rusqlite::Error::ToSqlConversionFailure(Box::new(SqliteManagerError::SerializationError(
                            e.to_string(),
                        )))
                    })?,
...
mod tests {
    ...
    use hanzo_messages::schemas::{
        ...
        hanzo_name::HanzoName,
        hanzo_tool_offering::{HanzoToolOffering, ToolPrice, UsageType, UsageTypeInquiry},
