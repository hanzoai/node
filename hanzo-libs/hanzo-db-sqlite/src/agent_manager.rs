use rusqlite::params;
use hanzo_messages::schemas::{llm_providers::agent::Agent, hanzo_name::HanzoName};

use crate::{SqliteManager, SqliteManagerError};

impl SqliteManager {
    pub fn add_agent(&self, agent: Agent, profile: &HanzoName) -> Result<(), SqliteManagerError> {
        let mut conn = self.get_connection()?;
        let tx = conn.transaction()?;

        let exists: bool = tx.query_row(
            "SELECT EXISTS(SELECT 1 FROM hanzo_agents WHERE agent_id = ?)",
            [&agent.agent_id],
            |row| row.get(0),
        )?;
        ...
        tx.execute(
            "INSERT INTO hanzo_agents (name, agent_id, full_identity_name, llm_provider_id, ui_description, knowledge, storage_path, tools, debug_mode, config, scope, tools_config_override, edited)
    ...
    pub fn remove_agent(&self, agent_id: &str) -> Result<(), SqliteManagerError> {
        ...
        tx.execute("DELETE FROM hanzo_agents WHERE agent_id = ?", [&agent_id])?;
    ...
    pub fn get_all_agents(&self) -> Result<Vec<Agent>, SqliteManagerError> {
        ...
        let mut stmt = conn.prepare("SELECT agent_id, name, full_identity_name, llm_provider_id, ui_description, knowledge, storage_path, tools, debug_mode, config, scope, tools_config_override, edited FROM hanzo_agents")?;
        let agents = stmt.query_map([], |row| {
            ...
            Ok(Agent {
                agent_id: row.get(0)?,
                name: row.get(1)?,
                full_identity_name: HanzoName::new(full_identity_name).map_err(|e| {
    ...
    pub fn get_agent(&self, agent_id: &str) -> Result<Option<Agent>, SqliteManagerError> {
        ...
        let mut stmt = conn.prepare("SELECT agent_id, name, full_identity_name, llm_provider_id, ui_description, knowledge, storage_path, tools, debug_mode, config, scope, tools_config_override, edited FROM hanzo_agents WHERE agent_id = ?")?;
...
mod tests {
    ...
    fn test_add_agent() {
        ...
        let agent = Agent {
            ...
            full_identity_name: HanzoName::new("did:hanzo:test_user/main".to_string()).unwrap(),
