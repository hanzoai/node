...
use hanzo_messages::schemas::indexable_version::IndexableVersion;
...
impl SqliteManager {
    ...
    pub async fn add_tool(&self, tool: HanzoTool) -> Result<HanzoTool, SqliteManagerError> {
        let model_type = self
    ...
    pub fn add_tool_with_vector(
        ...
        tool: HanzoTool,
        embedding: Vec<f32>,
    ) -> Result<HanzoTool, SqliteManagerError> {
        ...
        let exists: bool = tx.query_row(
            "SELECT EXISTS(SELECT 1 FROM hanzo_tools WHERE tool_key = ?1 AND version = ?2)",
        ...
        let (on_demand_price, is_network) = match tool_clone {
            HanzoTool::Network(ref network_tool, _) => {
        ...
        let (_, is_network) = match tool_clone {
            HanzoTool::Network(_, _) => (Some(0.0), true),
            _ => (None, false),
        };

        // Insert the embedding into the hanzo_tools_vec_items table with metadata
        tx.execute(
            "INSERT INTO hanzo_tools_vec_items (
                embedding, 
                is_enabled, 
                is_network, 
                tool_key
    ...
    pub async fn upgrade_tool(&self, new_tool: HanzoTool) -> Result<HanzoTool, SqliteManagerError> {
    ...
    pub fn upgrade_tool_with_vector(
        ...
        let (_old_config, upgraded): (Vec<ToolConfig>, HanzoTool) = match (old_tool, new_tool) {
            (HanzoTool::Deno(old_deno, _), HanzoTool::Deno(mut new_deno, is_enabled)) => {
                ...
                (old_config, HanzoTool::Deno(new_deno, is_enabled))
            }
            (HanzoTool::Network(old_network, _), HanzoTool::Network(mut new_network, is_enabled)) => {
                ...
                (old_config, HanzoTool::Network(new_network, is_enabled))
            }
            (HanzoTool::Python(old_python, _), HanzoTool::Python(mut new_python, is_enabled)) => {
                ...
                (old_config, HanzoTool::Python(new_python, is_enabled))
            }
            (HanzoTool::Rust(_old_rust, _), HanzoTool::Rust(new_rust, is_enabled)) => {
                (Vec::new(), HanzoTool::Rust(new_rust, is_enabled))
            }
    ...
    pub fn tool_vector_search_with_vector(
        ...
        // Retrieve the corresponding HanzoToolHeaders and pair with distances
    ...
    /// Retrieves a HanzoToolHeader based on its tool_key
    pub fn get_tool_header_by_key(&self, tool_key: &str) -> Result<HanzoToolHeader, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt =
            conn.prepare("SELECT tool_header FROM hanzo_tools WHERE tool_key = ?1 ORDER BY version DESC LIMIT 1")?;
    ...
    pub fn get_tool_by_agent_id(&self, agent_id: &str) -> Result<HanzoTool, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt =
            conn.prepare("SELECT tool_key FROM hanzo_tools WHERE tool_type = 'Agent' AND tool_key LIKE ?1")?;
        ...
        let tools = row.collect::<Result<Vec<Option<HanzoTool>>, _>>()?;
    ...
    /// Retrieves a HanzoTool based on its tool_key, sorted by descending version
    pub fn get_tool_by_key(&self, tool_key: &str) -> Result<HanzoTool, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt =
            conn.prepare("SELECT tool_data FROM hanzo_tools WHERE tool_key = ?1 ORDER BY version DESC LIMIT 1")?;
    ...
    // Updates a HanzoTool entry in the hanzo_tools table with a new embedding
    pub fn update_tool_with_vector(
        ...
        let rowid: i64 = tx
            .query_row(
                "SELECT rowid FROM hanzo_tools WHERE tool_key = ?1 AND version = ?2",
    ...
    /// Updates a HanzoTool entry by generating a new embedding if needed
    pub async fn update_tool(&self, tool: HanzoTool) -> Result<HanzoTool, SqliteManagerError> {
        let model_type = self
            .get_default_embedding_model()
            .unwrap_or_else(|_| EmbeddingModelType::default());
    ...
    pub fn get_all_tool_headers(&self) -> Result<Vec<HanzoToolHeader>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("SELECT tool_header FROM hanzo_tools")?;
    ...
    /// Removes one or all versions of a HanzoTool entry from the hanzo_tools table.
    ...
    pub fn remove_tool(&self, tool_key: &str, version: Option<String>) -> Result<(), SqliteManagerError> {
        ...
        let rowids: Vec<i64> = if let Some(ver_str) = version {
            ...
            let mut stmt = tx.prepare("SELECT rowid FROM hanzo_tools WHERE tool_key = ?1")?;
        ...
        tx.execute(
            "DELETE FROM invoice_network_errors WHERE invoice_id IN (
                SELECT invoice_id FROM invoices WHERE hanzo_offering_key = ?1
            )",
            params![tool_key_lower],
        )?;

        // Update any existing invoices to set hanzo_offering_key to NULL
        ...
        tx.execute(
            "UPDATE invoices SET hanzo_offering_key = NULL WHERE hanzo_offering_key = ?1",
            params![tool_key_lower],
        )?;
        ...
        // Remove tool playground entries (these reference hanzo_tools with cascade delete)
        // No explicit delete needed as ON DELETE CASCADE should handle it

        // Delete each row from hanzo_tools and hanzo_tools_vec_items
        for rowid in &rowids {
            tx.execute("DELETE FROM hanzo_tools WHERE rowid = ?1", params![rowid])?;
            tx.execute("DELETE FROM hanzo_tools_vec_items WHERE rowid = ?1", params![rowid])?;
        }
        ...
        for rowid in rowids {
            fts_conn.execute("DELETE FROM hanzo_tools_fts WHERE rowid = ?1", params![rowid])?;
        }

        Ok(())
    }

    /// Checks if the hanzo_tools table is empty
    ...
    /// Checks if a tool exists in the hanzo_tools table by its tool_key
    pub fn tool_exists(&self, tool_key: &str, version: Option<IndexableVersion>) -> Result<bool, SqliteManagerError> {
        ...
        let exists = match version {
            ...
            None => conn.query_row(
                "SELECT EXISTS(SELECT 1 FROM hanzo_tools WHERE tool_key = ?1)",
    ...
    /// Checks if there are any JS tools in the hanzo_tools table
    pub fn has_any_js_tools(&self) -> Result<bool, SqliteManagerError> {
        let conn = self.get_connection()?;
        let exists: bool = conn
            .query_row(
                "SELECT EXISTS(SELECT 1 FROM hanzo_tools WHERE tool_type = 'Deno')",
                [],
                |row| row.get(0),
            )
            .map_err(|e| {
    ...
    /// Checks if there are any Rust tools in the hanzo_tools table
    pub fn has_rust_tools(&self) -> Result<bool, SqliteManagerError> {
        let conn = self.get_connection()?;
        let count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM hanzo_tools WHERE tool_type = 'Rust'",
                [],
                |row| row.get(0),
            )
            .map_err(|e| {
    ...
    pub fn update_tools_fts(&self, tool: &HanzoTool) -> Result<(), SqliteManagerError> {
        ...
        tx.execute("DELETE FROM hanzo_tools_fts WHERE name = ?1", params![tool.name()])?;

        // Insert the updated tool name
        tx.execute("INSERT INTO hanzo_tools_fts(name) VALUES (?1)", params![tool.name()])?;
    ...
    pub fn search_tools_fts(&self, query: &str) -> Result<Vec<HanzoToolHeader>, SqliteManagerError> {
        // Get a connection from the in-memory pool for FTS operations
        let fts_conn = self
            .fts_pool
            .get()
        ...
        for phrase in phrases_to_search {
            let mut stmt = fts_conn.prepare("SELECT name FROM hanzo_tools_fts WHERE hanzo_tools_fts MATCH ?1")?;
    ...
    pub fn sync_tools_fts_table(&self) -> Result<(), SqliteManagerError> {
        // Use the pooled connection to access the hanzo_tools table
        let conn = self.get_connection()?;

        let mut stmt = conn.prepare("SELECT rowid, name FROM hanzo_tools")?;
        ...
        while let Some(row) = rows.next()? {
            ...
            fts_conn.execute(
                "INSERT INTO hanzo_tools_fts(rowid, name) VALUES (?1, ?2)",
                params![rowid, name],
            )?;
        }
        Ok(())
    ...
    pub fn update_tools_vector(
        ...
        tx.execute(
            "UPDATE hanzo_tools_vec_items SET 
                embedding = ?1,
                is_enabled = ?2,
                is_network = ?3
             WHERE tool_key = ?4",
    ...
    pub fn get_tool_by_key_and_version(
        ...
        let tool: HanzoTool = if let Some(version) = version {
            let version_number = version.get_version_number();
            conn.query_row(
                "SELECT tool_data FROM hanzo_tools WHERE tool_key = ?1 AND version = ?2",
            ...
            conn.query_row(
                "SELECT tool_data FROM hanzo_tools WHERE tool_key = ?1 ORDER BY version DESC LIMIT 1",
                params![tool_key_lower],
                |row| {
                    let tool_data: Vec<u8> = row.get(0)?;
                    serde_json::from_slice(&tool_data).map_err(|e| {
    ...
    /// Retrieves all HanzoTool entries that belong to a specific tool_set.
    pub fn get_tools_by_tool_set(&self, tool_set_name: &str) -> Result<Vec<HanzoTool>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("SELECT tool_data FROM hanzo_tools")?;
    ...
    pub fn get_all_tools_from_mcp_server(
        ...
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("SELECT tool_data FROM hanzo_tools WHERE tool_type = 'MCPServer' AND (json(tool_data) -> '$.content[0].mcp_server_ref') = '\"' || ? || '\"'")?;
    ...
    pub fn delete_all_tools_from_mcp_server(&self, mcp_server_id: String) -> Result<usize, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("DELETE FROM hanzo_tools WHERE tool_type = 'MCPServer' AND (json(tool_data) -> '$.content[0].mcp_server_ref') = '\"' || ? || '\"'")?;
...
mod tests {
    ...
    use hanzo_messages::schemas::hanzo_tool_offering::ToolPrice;
    use hanzo_messages::schemas::hanzo_tool_offering::UsageType;
    ...
    use hanzo_tools::tools::python_tools::PythonTool;
    use hanzo_tools::tools::tool_config::{BasicConfig, ToolConfig};
    use hanzo_tools::tools::tool_output_arg::ToolOutputArg;
    use hanzo_tools::tools::tool_types::OperatingSystem;
    use hanzo_tools::tools::tool_types::RunnerType;
    ...
    async fn test_add_deno_tool() {
        ...
        // Wrap the DenoTool in a HanzoTool::Deno variant
        let hanzo_tool = HanzoTool::Deno(deno_tool, true);

        // Debug: Print the tool before adding
        println!("Testing add_tool with: {:?}", hanzo_tool);

        let vector = SqliteManager::generate_vector_for_testing(0.1);

        // Add the tool to the database
        let result = manager.add_tool_with_vector(hanzo_tool.clone(), vector);
        assert!(result.is_ok());

        // Retrieve the tool using the new method
        let retrieved_tool = manager
            .get_tool_by_key(&hanzo_tool.tool_router_key().to_string_without_version())
            .unwrap();

        // Assert that the retrieved tool matches the added tool
        assert_eq!(retrieved_tool.name(), hanzo_tool.name());
        assert_eq!(retrieved_tool.description(), hanzo_tool.description());
        assert_eq!(retrieved_tool.author(), hanzo_tool.author());

        // Remove the tool from the database
        manager
            .remove_tool(&hanzo_tool.tool_router_key().to_string_without_version(), None)
            .unwrap();

        // Verify that the tool is removed from the hanzo_tools table
        let tool_removal_result = manager.get_tool_by_key(&hanzo_tool.tool_router_key().to_string_without_version());
        assert!(tool_removal_result.is_err());

        // Verify that the embedding is removed from the hanzo_tools_vec_items table
        let conn = manager.get_connection().unwrap();
        let embedding_removal_result: Result<i64, _> = conn.query_row(
            "SELECT rowid FROM hanzo_tools_vec_items WHERE rowid = ?1",
            params![hanzo_tool
                .tool_router_key()
    ...
    async fn test_tool_vector_search() {
        ...
        let hanzo_tool_1 = HanzoTool::Deno(deno_tool_1, true);
        let hanzo_tool_2 = HanzoTool::Deno(deno_tool_2, true);
        let hanzo_tool_3 = HanzoTool::Deno(deno_tool_3, true);

        // Add the tools to the database with different vectors
        manager
            .add_tool_with_vector(hanzo_tool_1.clone(), SqliteManager::generate_vector_for_testing(0.1))
            .unwrap();
        manager
            .add_tool_with_vector(hanzo_tool_2.clone(), SqliteManager::generate_vector_for_testing(0.5))
            .unwrap();
        manager
            .add_tool_with_vector(hanzo_tool_3.clone(), SqliteManager::generate_vector_for_testing(0.9))
            .unwrap();
        ...
        let num_results = 1;
        let search_results: Vec<HanzoToolHeader> = manager
    ...
    async fn test_update_middle_tool() {
        ...
        // Wrap the DenoTools in HanzoTool::Deno variants
        let hanzo_tool_1 = HanzoTool::Deno(deno_tool_1, true);
        let hanzo_tool_2 = HanzoTool::Deno(deno_tool_2, true);
        let hanzo_tool_3 = HanzoTool::Deno(deno_tool_3, true);
        ...
        manager
            ...
            .unwrap();
        manager
            .add_tool_with_vector(hanzo_tool_2.clone(), SqliteManager::generate_vector_for_testing(0.2))
            .unwrap();
        manager
            .add_tool_with_vector(hanzo_tool_3.clone(), SqliteManager::generate_vector_for_testing(0.3))
            .unwrap();
        ...
        let mut updated_tool_2 = hanzo_tool_2.clone();
        if let HanzoTool::Deno(ref mut deno_tool, _) = updated_tool_2 {
        ...
        // Manually query the hanzo_tools_vec_items table to verify the vector
        let conn = manager.get_connection().unwrap();
        let rowid: i64 = conn
            .query_row(
                "SELECT rowid FROM hanzo_tools WHERE tool_key = ?1",
    ...
    async fn test_add_duplicate_tool() {
        ...
        // Add the tool to the database
        let vector = SqliteManager::generate_vector_for_testing(0.1);
        let result = manager.add_tool_with_vector(hanzo_tool.clone(), vector.clone());
        assert!(result.is_ok());

        // Attempt to add the same tool again
        let duplicate_result = manager.add_tool_with_vector(hanzo_tool.clone(), vector);
    ...
    async fn test_fts_search() {
        ...
        for (i, tool) in tools.into_iter().enumerate() {
            let hanzo_tool = HanzoTool::Deno(tool, true);
            let vector = SqliteManager::generate_vector_for_testing(0.1 * (i + 1) as f32);
            if let Err(e) = manager.add_tool_with_vector(hanzo_tool, vector) {
    ...
    async fn test_tool_vector_search_with_disabled() {
        ...
        let hanzo_enabled = HanzoTool::Deno(enabled_tool, true);
        let hanzo_disabled = HanzoTool::Deno(disabled_tool, false);

        manager
            .add_tool_with_vector(hanzo_enabled.clone(), SqliteManager::generate_vector_for_testing(0.1))
            .unwrap();
        manager
            .add_tool_with_vector(
                hanzo_disabled.clone(),
        ...
        if let HanzoTool::Deno(mut deno_tool, _is_enabled) = hanzo_enabled {
            deno_tool.activated = false;
            let updated_tool = HanzoTool::Deno(deno_tool, false);
    ...
    async fn test_tool_vector_search_with_network_filter() {
        ...
        // Wrap the tools in HanzoTool variants
        let hanzo_enabled_non_network = HanzoTool::Deno(enabled_non_network_tool, true);
        let hanzo_disabled_non_network = HanzoTool::Deno(disabled_non_network_tool, false);
        let hanzo_enabled_network = HanzoTool::Network(enabled_network_tool, true);

        // Add the tools to the database
        manager
            .add_tool_with_vector(
                hanzo_enabled_non_network.clone(),
                SqliteManager::generate_vector_for_testing(0.1),
            )
            .unwrap();
        manager
            .add_tool_with_vector(
                hanzo_disabled_non_network.clone(),
                SqliteManager::generate_vector_for_testing(0.2),
            )
            .unwrap();
        manager
            .add_tool_with_vector(
                hanzo_enabled_network.clone(),
    ...
    async fn test_tool_vector_search_with_vector_limited() {
        ...
        let hanzo_tool1 = HanzoTool::Deno(tool1, true);
        let hanzo_tool2 = HanzoTool::Deno(tool2, true);
        let hanzo_tool3 = HanzoTool::Deno(tool3, true);

        // Tool 2 will have the closest vector to our search query
        manager
            .add_tool_with_vector(hanzo_tool1.clone(), SqliteManager::generate_vector_for_testing(0.1))
            .unwrap();
        manager
            .add_tool_with_vector(hanzo_tool2.clone(), SqliteManager::generate_vector_for_testing(0.5))
            .unwrap();
        manager
            .add_tool_with_vector(hanzo_tool3.clone(), SqliteManager::generate_vector_for_testing(0.9))
            .unwrap();
        ...
        let limited_tool_keys = vec![
            hanzo_tool1.tool_router_key().to_string_without_version(),
            hanzo_tool3.tool_router_key().to_string_without_version(),
        ];
    ...
    async fn test_add_tools_with_different_versions() {
        ...
        // Wrap the DenoTools in HanzoTool::Deno variants
        let hanzo_tool_v1 = HanzoTool::Deno(deno_tool_v1, true);
        let hanzo_tool_v2 = HanzoTool::Deno(deno_tool_v2, true);

        // Add both tools to the database
        manager
            .add_tool_with_vector(hanzo_tool_v1.clone(), SqliteManager::generate_vector_for_testing(0.1))
            .unwrap();
        manager
            .add_tool_with_vector(hanzo_tool_v2.clone(), SqliteManager::generate_vector_for_testing(0.2))
            .unwrap();

        // Retrieve and verify both tools are added
        let retrieved_tool_v1 = manager
            .get_tool_by_key(&hanzo_tool_v1.tool_router_key().to_string_without_version())
            .unwrap();
        let retrieved_tool_v2 = manager
            .get_tool_by_key(&hanzo_tool_v2.tool_router_key().to_string_without_version())
            .unwrap();
        ...
        let retrieved_tool_v1_0 = manager
            .get_tool_by_key_and_version(
                &hanzo_tool_v1.tool_router_key().to_string_without_version(),
                Some(version_1_0),
            )
            .unwrap();
        ...
        let retrieved_tool_latest = manager
            .get_tool_by_key_and_version(&hanzo_tool_v1.tool_router_key().to_string_without_version(), None)
            .unwrap();
    ...
    async fn test_upgrade_tool_preserves_config() {
        ...
        let hanzo_tool_v1 = HanzoTool::Deno(deno_tool_v1.clone(), true);
        let vector_v1 = SqliteManager::generate_vector_for_testing(0.1);
        manager
            .add_tool_with_vector(hanzo_tool_v1.clone(), vector_v1)
            .unwrap();
        ...
        let hanzo_tool_v2 = HanzoTool::Deno(deno_tool_v2.clone(), true);

        // Upgrade to version 2.0.0
        let vector_v2 = SqliteManager::generate_vector_for_testing(0.2);
        let upgraded = manager
            .upgrade_tool_with_vector(hanzo_tool_v2.clone(), vector_v2)
            .unwrap();
        ...
        if let HanzoTool::Deno(new_tool, _) = retrieved {
    ...
    async fn test_upgrade_tool_preserves_config_python() {
        ...
        let hanzo_tool_v1 = HanzoTool::Python(python_tool_v1, true);
        manager
            .add_tool_with_vector(hanzo_tool_v1, SqliteManager::generate_vector_for_testing(0.1))
            .unwrap();
        ...
        let hanzo_tool_v2 = HanzoTool::Python(python_tool_v2, true);
        let upgraded = manager
            .upgrade_tool_with_vector(hanzo_tool_v2, SqliteManager::generate_vector_for_testing(0.2))
            .unwrap();
        ...
        if let HanzoTool::Python(new_tool, _) = retrieved {
    ...
    async fn test_get_tools_by_tool_set() {
        ...
        let hanzo_tool1 = HanzoTool::Deno(tool1, true);
        let hanzo_tool2 = HanzoTool::Deno(tool2, true);
        let hanzo_tool3 = HanzoTool::Python(tool3, true);
        let hanzo_tool4 = HanzoTool::Deno(tool4, true);

        manager
            .add_tool_with_vector(hanzo_tool1, SqliteManager::generate_vector_for_testing(0.1))
            .unwrap();
        manager
            .add_tool_with_vector(hanzo_tool2, SqliteManager::generate_vector_for_testing(0.2))
            .unwrap();
        manager
            .add_tool_with_vector(hanzo_tool3, SqliteManager::generate_vector_for_testing(0.3))
            .unwrap();
        manager
            .add_tool_with_vector(hanzo_tool4, SqliteManager::generate_vector_for_testing(0.4))
            .unwrap();
    ...
    async fn test_set_common_toolset_config() {
        ...
        // Add tools
        let hanzo_tool1 = HanzoTool::Deno(tool1, true);
        let hanzo_tool2 = HanzoTool::Python(tool2, true);
        let hanzo_tool3 = HanzoTool::Deno(tool3, true);

        manager.add_tool(hanzo_tool1.clone()).await.unwrap();
        manager.add_tool(hanzo_tool2.clone()).await.unwrap();
        manager.add_tool(hanzo_tool3.clone()).await.unwrap();
        ...
        assert!(updated_keys_set.contains(&hanzo_tool1.tool_router_key().to_string_without_version()));
        assert!(updated_keys_set.contains(&hanzo_tool2.tool_router_key().to_string_without_version()));

        // Verify Tool 1 config
        let updated_tool1 = manager
            .get_tool_by_key(&hanzo_tool1.tool_router_key().to_string_without_version())
            .unwrap();
        if let HanzoTool::Deno(t, _) = updated_tool1 {
        ...
        let updated_tool2 = manager
            .get_tool_by_key(&hanzo_tool2.tool_router_key().to_string_without_version())
            .unwrap();
        if let HanzoTool::Python(t, _) = updated_tool2 {
            let api_key_config = t.config.iter().find(|c| c.name() == "api_key").unwrap();
            assert_eq!(api_key_config.header(), json!("new_common_key"));
        ...
        let updated_tool3 = manager
            .get_tool_by_key(&hanzo_tool3.tool_router_key().to_string_without_version())
            .unwrap();
        if let HanzoTool::Deno(t, _) = updated_tool3 {
            let api_key_config = t.config.iter().find(|c| c.name() == "api_key").unwrap();
    ...
    async fn test_add_duplicate_python_tool() {
        ...
        // Wrap the PythonTool in a HanzoTool::Python variant
        let hanzo_tool = HanzoTool::Python(python_tool_data, true);
    ...
    // Helper function to create an MCPServerTool wrapped in HanzoTool
    fn create_mcp_hanzo_tool(name: &str, mcp_server_id: String, profile_name: &str, version: &str) -> HanzoTool {
        ...
        HanzoTool::MCPServer(mcp_tool_data, true)
    }

    // Helper for Deno tool
    fn create_deno_hanzo_tool(name: &str, author: &str, version: &str) -> HanzoTool {
        let tool_router_key = ToolRouterKey::new(
        ...
        HanzoTool::Deno(deno_tool_data, true)
    }
    ...
    async fn test_get_all_tools_from_mcp_server() {
        ...
        // Tools are still created with Uuid for mcp_server_ref via create_mcp_hanzo_tool
        let tool1_s1 = create_mcp_hanzo_tool(
            "Tool1S1",
            mcp_server1.id.unwrap_or_default().to_string(),
            "profile1",
            "1.0",
        );
        let tool2_s1 = create_mcp_hanzo_tool(
            "Tool2S1",
            mcp_server1.id.unwrap_or_default().to_string(),
            "profile1",
            "1.0",
        );
        let tool1_s2 = create_mcp_hanzo_tool(
            "Tool1S2",
            mcp_server2.id.unwrap_or_default().to_string(),
            "profile2",
            "1.0",
        );
        let deno_tool = create_deno_hanzo_tool("MyDenoTool", "DenoAuthor", "1.0");
    ...
    async fn test_delete_all_tools_from_mcp_server() {
        ...
        let tool1_del = create_mcp_hanzo_tool(
            "MCPTool1ToDel",
        ...
        let tool2_del = create_mcp_hanzo_tool(
            "MCPTool2ToDel",
            mcp_server_del.id.unwrap_or_default().to_string(),
            "profile_del",
            "1.0",
        ...
        let tool_keep_mcp = create_mcp_hanzo_tool(
            "MCPToolToKeep",
        ...
        let deno_tool_keep = create_deno_hanzo_tool("DenoToolToKeep", "DenoAuthor", "1.0");
