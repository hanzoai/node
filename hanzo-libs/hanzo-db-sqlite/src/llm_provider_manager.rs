...
use hanzo_messages::schemas::{
    llm_providers::serialized_llm_provider::SerializedLLMProvider, hanzo_name::HanzoName,
};
...
impl SqliteManager {
    ...
    pub fn remove_llm_provider(&self, llm_provider_id: &str, profile: &HanzoName) -> Result<(), SqliteManagerError> {
        let conn = self.get_connection()?;
    ...
    pub fn get_llm_provider_profiles_with_access(
        ...
        let rows = stmt.query_map(params![&llm_provider_id], |row| {
            ...
            HanzoName::new(full_identity_name).map_err(|e| {
    ...
    fn db_llm_provider_id(llm_provider_id: &str, profile: &HanzoName) -> Result<String, SqliteManagerError> {
...
mod tests {
    ...
    fn test_get_llm_providers_for_profile() {
        ...
        let identity1 = HanzoName::new("did:hanzo:alice/profileName1/agent/myChatGPTAgent".to_string()).unwrap();
        let identity2 = HanzoName::new("did:hanzo:bob/profileName2/agent/myChatGPTAgent2".to_string()).unwrap();
