...
lazy_static! {
    pub static ref DEFAULT_EMBEDDINGS_SERVER_URL: &'static str = "https://api.hanzo.ai";
...
impl RemoteEmbeddingGenerator {
    ...
    /// String of the main endpoint url for generating embeddings via the
    /// native hanzo-engine OpenAI-compatible API (`/v1/embeddings`).
    ...
    pub async fn generate_embedding_ollama(
        ...
        loop {
            // Prepare the request body (native hanzo-engine OpenAI shape).
                    ...
                    match embedding_response {
                        Ok(embedding_response) => {
                            match embedding_response.data.into_iter().next() {
                                Some(d) => return Ok(d.embedding),
                                None => {
                                    return Err(HanzoEmbeddingError::RequestFailed(
                                        "Embedding API returned no data".to_string(),
                                    ))
                                }
                            }
                        }
    ...
    fn generate_embedding_ollama_blocking(&self, input_string: &str) -> Result<Vec<f32>, HanzoEmbeddingError> {
        ...
        // Build the request
        let mut request = client
            .post(&format!("{}", self.embeddings_endpoint_url()))
            .header("Content-Type", "application/json")
            .json(&request_body);
        ...
        if response.status().is_success() {
            ...
            embedding_response
                ...
                .map(|d| d.embedding)
                .ok_or_else(|| {
                    HanzoEmbeddingError::RequestFailed("Embedding API returned no data".to_string())
                })
        } else {
