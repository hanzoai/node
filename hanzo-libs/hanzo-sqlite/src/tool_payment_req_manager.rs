use rusqlite::params;
use hanzo_message_primitives::schemas::hanzo_tool_offering::{HanzoToolOffering, UsageType};
...
mod tests {
    ...
    async fn test_remove_tool_offering_with_invoices() {
        ...
        use hanzo_message_primitives::schemas::wallet_mixed::PublicAddress;
        use rusqlite::params;
        ...
        let mut stmt = conn.prepare("SELECT invoice_id, hanzo_offering_key FROM invoices WHERE invoice_id = ?1").unwrap();
        ...
        match result {
            Ok((invoice_id, offering_key)) => {
                ...
                assert!(offering_key.is_none(), "hanzo_offering_key should be NULL after tool offering removal");
