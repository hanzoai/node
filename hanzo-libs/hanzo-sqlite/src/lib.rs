...
impl SqliteManager {
    ...
    pub async fn migrate_embeddings_to_new_model(
        &self,
        embedding_generator: &dyn hanzo_embedding::embedding_generator::EmbeddingGenerator,
        ...
        let prompts: Vec<CustomPrompt> = {
            let conn = self.get_connection()?;
            let mut stmt = conn.prepare("SELECT id, name, is_system, is_enabled, version, prompt, is_favorite FROM hanzo_prompts")?;
