...
impl SqliteManager {
    ...
    pub fn set_tool_playground(&self, tool: &ToolPlayground) -> Result<(), SqliteManagerError> {
        ...
        let tool_version: i64 = tx.query_row(
            "SELECT version FROM hanzo_tools 
             WHERE tool_key = ?1 
...
mod tests {
    ...
    use hanzo_message_primitives::schemas::tool_router_key::ToolRouterKey;
    use hanzo_tools_primitives::tools::{
        deno_tools::DenoTool, parameters::Parameters, hanzo_tool::HanzoTool, tool_output_arg::ToolOutputArg, tool_types::{OperatingSystem, RunnerType, ToolResult}
    };
    ...
    async fn add_tool_to_db(manager: &mut SqliteManager) -> String {
        ...
        manager.add_tool_with_vector(hanzo_tool.clone(), vector).unwrap();
    ...
    async fn test_add_and_remove_tool_playground_message() {
        ...
        let tool_playground = create_test_tool_playground(hanzo_tool.tool_router_key().to_string_without_version());
        manager.set_tool_playground(&tool_playground).unwrap();
        ...
        manager
            .remove_tool_playground(&hanzo_tool.tool_router_key().to_string_without_version())
            .unwrap();

        // Verify the ToolPlayground is removed
        let result = manager.get_tool_playground(&hanzo_tool.tool_router_key().to_string_without_version());
        assert!(matches!(result, Err(SqliteManagerError::ToolPlaygroundNotFound(_))));
