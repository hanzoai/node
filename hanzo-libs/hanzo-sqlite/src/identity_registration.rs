...
use hanzo_message_primitives::{
    schemas::{
        identity::{DeviceIdentity, StandardIdentity, StandardIdentityType}, identity_registration::{RegistrationCodeInfo, RegistrationCodeStatus}, hanzo_name::{HanzoName, HanzoSubidentityType}
    }, hanzo_message::hanzo_message_schemas::{IdentityPermissions, RegistrationCodeType}, hanzo_utils::{
...
impl SqliteManager {
    ...
    pub fn main_profile_exists(&self, node_name: &str) -> Result<bool, SqliteManagerError> {
        ...
        let current_identity_name =
            match HanzoName::from_node_and_profile_names(node_name.to_string(), profile_name.to_lowercase()) {
                Ok(name) => name,
                Err(_) => {
    ...
    pub fn use_registration_code(
        ...
        match code_info.code_type {
            RegistrationCodeType::Profile => {
                let current_identity_name =
                    match HanzoName::from_node_and_profile_names(node_name.to_string(), new_name.to_lowercase()) {
                        Ok(name) => name,
                        Err(_) => {
                            return Err(SqliteManagerError::InvalidIdentityName(format!(
                                "{}/{}",
                ...
                match self.get_profile(current_identity_name.clone())? {
                    None => {
                        ...
                        let full_identity_name =
                            match HanzoName::from_node_and_profile_names(node_name.to_string(), new_name.to_string())
                            {
                                Ok(name) => name,
                                Err(_) => {
                                    return Err(SqliteManagerError::InvalidIdentityName(format!(
            ...
            RegistrationCodeType::Device(profile_name) => {
                let current_identity_name = match HanzoName::from_node_and_profile_names(
                    node_name.to_string(),
                ...
                let full_identity_name = match HanzoName::from_node_and_profile_names_and_type_and_name(
                    node_name.to_string(),
    ...
    pub fn update_local_node_keys(
        &self,
        my_node_identity_name: HanzoName,
