...
use hanzo_message_primitives::{
    schemas::hanzo_fs::{ParsedFile, HanzoFileChunk}, hanzo_utils::hanzo_path::HanzoPath
};
...
impl SqliteManager {
    ...
    pub fn get_chunk_with_embedding(
        &self,
        chunk_id: i64,
    ) -> Result<Option<(HanzoFileChunk, Option<Vec<f32>>)>, SqliteManagerError> {
        ...
        let row_result = stmt.query_row([chunk_id], |row| {
            ...
            // Build the chunk
            let chunk_struct = HanzoFileChunk {
                chunk_id: Some(c_id),
                parsed_file_id,
                position,
                content,
    ...
    pub fn get_chunks_for_parsed_file(&self, parsed_file_id: i64) -> Result<Vec<HanzoFileChunk>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare(
    ...
    ) -> Result<Vec<HanzoFileChunk>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare(
    ...
    pub fn search_chunks(
        ...
        limit: usize,
    ) -> Result<Vec<(HanzoFileChunk, f64)>, SqliteManagerError> {
        let conn = self.get_connection()?;
        ...
        for (chunk_id, distance) in chunk_ids_and_distances {
            if let Some(chunk) = self.get_chunk_with_embedding(chunk_id)? {
                results.push((chunk.0, distance)); // Assuming get_chunk_with_embedding returns (HanzoFileChunk,
                                                   // Option<Vec<f32>>)
            }
        }
    ...
    pub fn get_parsed_file_by_hanzo_path(
        &self,
        path: &HanzoPath,
