...
impl SqliteManager {
    pub fn save_wallet_manager(&self, wallet_data: &Value) -> Result<(), SqliteManagerError> {
        ...
        let mut stmt = conn.prepare(
            "INSERT INTO hanzo_wallet (id, wallet_data)
                VALUES (1, ?)
                ON CONFLICT (id)
                DO UPDATE SET
    ...
    pub fn read_wallet_manager(&self) -> Result<Value, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("SELECT wallet_data FROM hanzo_wallet LIMIT 1")?;
...
mod tests {
    ...
    async fn test_save_and_read_wallet_manager() {
        ...
        // Verify that hanzo_wallet table has only one row
        let conn = manager.get_connection().unwrap();
        let mut stmt = conn.prepare("SELECT COUNT(*) FROM hanzo_wallet").unwrap();
