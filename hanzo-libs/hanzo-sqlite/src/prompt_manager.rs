...
impl SqliteManager {
    ...
    // Adds a CustomPrompt entry to the hanzo_prompts table and its vector to prompt_vec_items
    pub fn add_prompt_with_vector(&self, prompt: &CustomPrompt, vector: Vec<f32>) -> Result<CustomPrompt> {
        let mut conn = self.get_connection()?;
        let tx = conn.transaction()?;

        tx.execute(
            "INSERT INTO hanzo_prompts (
                name,
                is_system,
                is_enabled,
                version,
    ...
    pub fn get_prompts(
        ...
        let mut query = "SELECT * FROM hanzo_prompts WHERE 1=1".to_string();
    ...
    pub fn get_prompt(&self, rowid: i64) -> Result<Option<CustomPrompt>> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("SELECT * FROM hanzo_prompts WHERE rowid = ?1 LIMIT 1")?;
    ...
    pub fn update_prompt_with_vector(&self, prompt: &CustomPrompt, vector: Vec<f32>) -> Result<(), SqliteManagerError> {
        ...
        if let Some(_existing_prompt) = Self::get_prompt(self, prompt.rowid.unwrap())? {
            ...
            let mut stmt = tx.prepare("SELECT rowid FROM hanzo_prompts WHERE name = ?1")?;
    ...
    pub fn get_favorite_prompts(&self) -> Result<Vec<CustomPrompt>> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("SELECT * FROM hanzo_prompts WHERE is_favorite = 1")?;
    ...
    pub fn remove_prompt(&self, name: &str) -> Result<()> {
        ...
        let rowid: Option<i64> = tx
            .query_row(
                "SELECT rowid FROM hanzo_prompts WHERE name = ?1",
                params![name],
                |row| row.get(0),
            )
            .optional()?;
        ...
        if let Some(rowid) = rowid {
            // Delete the prompt from hanzo_prompts
            tx.execute("DELETE FROM hanzo_prompts WHERE rowid = ?1", params![rowid])?;
    ...
    pub async fn update_prompts_fts(&self, prompt: &CustomPrompt) -> Result<(), SqliteManagerError> {
        ...
        tx.execute("DELETE FROM hanzo_prompts_fts WHERE name = ?1", params![prompt.name])?;

        // Insert the updated prompt name
        tx.execute(
            "INSERT INTO hanzo_prompts_fts(name) VALUES (?1)",
            params![prompt.name],
        )?;
    ...
    pub fn search_prompts_by_name(&self, query: &str) -> Result<Vec<CustomPrompt>, SqliteManagerError> {
        ...
        let mut stmt = fts_conn.prepare("SELECT name FROM hanzo_prompts_fts WHERE hanzo_prompts_fts MATCH ?1")?;
        ...
        for name_result in name_iter {
            ...
            let mut stmt = conn.prepare("SELECT * FROM hanzo_prompts WHERE name = ?1")?;
    ...
    pub fn sync_prompts_fts_table(&self) -> Result<(), SqliteManagerError> {
        // Use the pooled connection to access the hanzo_prompts table
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("SELECT rowid, name FROM hanzo_prompts")?;
        ...
        while let Some(row) = rows.next()? {
            ...
            fts_conn.execute("DELETE FROM hanzo_prompts_fts WHERE rowid = ?1", params![rowid])?;

            // Insert the new entry
            fts_conn.execute(
                "INSERT INTO hanzo_prompts_fts(rowid, name) VALUES (?1, ?2)",
                params![rowid, name],
            )?;
        }
        Ok(())
