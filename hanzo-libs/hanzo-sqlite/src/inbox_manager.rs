...
use hanzo_message_primitives::{
    schemas::{
        identity::StandardIdentity, inbox_name::InboxName, inbox_permission::InboxPermission, job_config::JobConfig, hanzo_name::HanzoName, smart_inbox::{LLMProviderSubset, ProviderType, SmartInbox}, ws_types::{WSMessageType, WSUpdateHandler}
    }, hanzo_message::{
        hanzo_message::{NodeApiData, HanzoMessage}, hanzo_message_schemas::WSTopic
    }, hanzo_utils::hanzo_time::HanzoStringTime
};
...
impl SqliteManager {
    pub fn create_empty_inbox(&self, inbox_name: String, is_hidden: Option<bool>) -> Result<(), SqliteManagerError> {
        ...
        conn.execute(
            ...
            params![
                inbox_name,
                smart_inbox_name,
                HanzoStringTime::generate_time_now(),
                is_hidden
            ],
        )?;
        Ok(())
    ...
    pub async fn unsafe_insert_inbox_message(
        ...
        let mut time_key = match ext_metadata.scheduled_time.is_empty() {
            true => HanzoStringTime::generate_time_now(),
        ...
        if let InboxName::JobInbox { .. } = inbox_name_manager {
            if let Some(parent_key) = &parent_key.clone() {
                ...
                if parsed_time_key < parsed_parent_time {
                    time_key = HanzoStringTime::generate_time_now();
                }
            }
        }
            ...
            .map_err(|e| SqliteManagerError::SomeError(e.to_string()))?;
        {
            ...
            tx.execute(
            "INSERT OR REPLACE INTO inbox_messages (message_hash, inbox_name, hanzo_message, parent_message_hash, time_key) VALUES (?1, ?2, ?3, ?4, ?5)",
    ...
    pub fn fetch_message_and_hash(&self, hash_key: &str) -> Result<(HanzoMessage, String), SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("SELECT hanzo_message FROM inbox_messages WHERE message_hash = ?1")?;
        let mut rows = stmt.query(params![hash_key])?;

        let encoded_message: Vec<u8> = rows.next()?.ok_or(SqliteManagerError::DataNotFound)?.get(0)?;
        let message = HanzoMessage::decode_message_result(encoded_message)
            .map_err(|e| SqliteManagerError::SomeError(e.to_string()))?;
    ...
    pub fn get_last_messages_from_inbox(
    ...
    ) -> Result<Vec<Vec<HanzoMessage>>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare(
            "SELECT message_hash, parent_message_hash, hanzo_message FROM inbox_messages
                WHERE inbox_name = ?1
        ...
        let message_rows = stmt.query_map(params![inbox_name], |row| {
            ...
            let message = HanzoMessage::decode_message_result(encoded_message).map_err(|e| {
    ...
    pub fn get_last_unread_messages_from_inbox(
    ...
    ) -> Result<Vec<HanzoMessage>, SqliteManagerError> {
        ...
        // Flatten the Vec<Vec<HanzoMessage>> to Vec<HanzoMessage>
        let messages: Vec<HanzoMessage> = messages.into_iter().flatten().collect();
    ...
    pub fn add_permission(
        ...
        let hanzo_profile = identity
            .full_identity_name
            .extract_profile()
            .map_err(|_| SqliteManagerError::InvalidProfileName(identity.full_identity_name.to_string()))?;
        self.add_permission_with_profile(inbox_name, hanzo_profile, perm)
    }
    ...
    pub fn get_last_messages_from_all(&self, n: usize) -> Result<Vec<HanzoMessage>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare(
            "SELECT hanzo_message FROM inbox_messages
                ORDER BY time_key DESC
                LIMIT ?1",
        )?;
    ...
    pub fn get_first_message_from_inbox(
        &self,
        inbox_name: String,
    ) -> Result<Option<HanzoMessage>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare(
            "SELECT hanzo_message FROM inbox_messages 
            WHERE inbox_name = ?1 
...
mod tests {
    ...
    use hanzo_message_primitives::{
        schemas::identity::StandardIdentityType, hanzo_message::{
            hanzo_message::MessageBody, hanzo_message_schemas::{IdentityPermissions, MessageSchemaType}
        }, hanzo_utils::{
            encryption::{unsafe_deterministic_encryption_keypair, EncryptionMethod}, job_scope::MinimalJobScope, search_mode::VectorSearchMode, hanzo_message_builder::HanzoMessageBuilder, signatures::{clone_signature_secret_key, unsafe_deterministic_signature_keypair}
        }
    };
    ...
    async fn test_insert_two_messages_and_check_order_and_parent() {
        ...
        let expected_parent_hash = if let MessageBody::Unencrypted(hanzo_body) = &messages[0][0].body {
            hanzo_body
                .internal_metadata
                .node_api_data
                .as_ref()
        ...
        let actual_parent_hash = if let MessageBody::Unencrypted(hanzo_body) = &messages[1][0].body {
            hanzo_body
                .internal_metadata
                .node_api_data
                .as_ref()
    ...
    async fn db_inbox() {
        ...
        let result = HanzoMessage::decode_message_result(message.encode_message().unwrap());
    ...
    async fn benchmark_get_all_smart_inboxes_for_profile() {
        ...
        let profile_identity = StandardIdentity::new(
            HanzoName::from_node_and_profile_names(node_identity_name.to_string(), subidentity_name.to_string())
                .unwrap(),
            None,
