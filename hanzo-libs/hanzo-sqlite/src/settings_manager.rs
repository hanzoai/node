...
impl SqliteManager {
    pub fn get_supported_embedding_models(&self) -> Result<Vec<EmbeddingModelType>, SqliteManagerError> {
        ...
        let mut stmt = conn.prepare("SELECT value FROM hanzo_settings WHERE key = 'supported_embedding_models'")?;
    ...
    pub fn update_supported_embedding_models(&self, models: Vec<EmbeddingModelType>) -> Result<(), SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn
            .prepare("INSERT OR REPLACE INTO hanzo_settings (key, value) VALUES ('supported_embedding_models', ?)")?;
    ...
    pub fn set_api_v2_key(&self, key: &str) -> Result<(), SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("INSERT OR REPLACE INTO hanzo_settings (key, value) VALUES ('api_v2_key', ?)")?;
        stmt.execute([key])?;
    ...
    pub fn read_api_v2_key(&self) -> Result<Option<String>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let stmt = conn.prepare("SELECT value FROM hanzo_settings WHERE key = 'api_v2_key'");
    ...
    pub fn set_ngrok_auth_token(&self, auth_token: Option<&str>) -> Result<(), SqliteManagerError> {
        ...
        if let Some(auth_token) = auth_token {
            let mut stmt = conn.prepare("INSERT OR REPLACE INTO hanzo_settings (key, value) VALUES ('ngrok_auth_token', ?)")?;
            stmt.execute([auth_token])?;
        } else {
            let mut stmt = conn.prepare("DELETE FROM hanzo_settings WHERE key = 'ngrok_auth_token'")?;
            stmt.execute([])?;
        }
    ...
    pub fn read_ngrok_auth_token(&self) -> Result<Option<String>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let stmt = conn.prepare("SELECT value FROM hanzo_settings WHERE key = 'ngrok_auth_token'");
