...
impl From<HanzoNameError> for InboxNameError {
    fn from(error: HanzoNameError) -> Self {
        InboxNameError::HanzoNameError(error)
    }
}
...
impl InboxName {
    pub fn new(inbox_name: String) -> Result<Self, InboxNameError> {
        ...
        if parts[0] == "inbox" {
            ...
            for part in &parts[1..parts.len() - 1] {
                if !HanzoName::is_fully_valid(part.to_string()) {
                    return Err(InboxNameError::InvalidFormat(inbox_name.clone()));
                }
                match HanzoName::new(part.to_string()) {
    ...
    pub fn from_message(message: &HanzoMessage) -> Result<InboxName, InboxNameError> {
        match &message.body {
    ...
    pub fn has_creation_access(&self, identity_name: HanzoName) -> Result<bool, InboxNameError> {
    ...
    pub fn get_regular_inbox_name_from_params(
        ...
        let sender_name = HanzoName::new(sender_full.clone())
            .map_err(|_| HanzoNameError::InvalidNameFormat(sender_full.to_string()))?;
        let recipient_name = HanzoName::new(recipient_full.clone())
            .map_err(|_| HanzoNameError::InvalidNameFormat(recipient_full.to_string()))?;
...
mod tests {
    ...
    fn test_from_message() {
        let mock_message = HanzoMessage {
            body: MessageBody::Unencrypted(HanzoBody {
                message_data: MessageData::Unencrypted(HanzoData {
    ...
    fn test_has_creation_access() {
        ...
        let identity_name = HanzoName::new("@@node1.shinkai/subidentity".to_string()).unwrap();
        let identity_name_2 = HanzoName::new("@@node2.shinkai/subidentity".to_string()).unwrap();
