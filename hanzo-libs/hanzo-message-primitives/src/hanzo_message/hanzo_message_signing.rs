...
impl HanzoMessage {
    ...
    pub fn sign_inner_layer(&mut self, secret_key: &SigningKey) -> Result<(), HanzoMessageError> {
        // Calculate the hash of the HanzoBody with an empty inner signature
        ...
        match &mut self.body {
            ...
            _ => {
                return Err(HanzoMessageError::SigningError(
    ...
    pub fn verify_inner_layer_signature(
        ...
        // Convert the MessageBody to a HanzoBody
        let hanzo_body = match &self.body {
        ...
        // Get the signature from hanzo_body.internal_metadata.signature
    ...
    pub fn calculate_message_hash_with_empty_inner_signature(&self) -> Result<String, HanzoMessageError> {
        ...
        // Hash the HanzoBody
        let mut hasher = Hasher::new();
        hasher.update(hanzo_body_string.as_bytes());
        let hanzo_body_hash = hasher.finalize();

        Ok(hex::encode(hanzo_body_hash.as_bytes()))
    }

    pub fn inner_content_ready_for_hashing(&self) -> Result<String, HanzoMessageError> {
        ...
        // Prepare HanzoBody for hashing - set signature to empty
        let mut hanzo_body_for_hashing = hanzo_body.clone();
        hanzo_body_for_hashing.internal_metadata.signature = String::from("");
        hanzo_body_for_hashing.internal_metadata.node_api_data = None;

        // Sort the JSON Value using the helper function
        let sorted_json = Self::to_sorted_json(&hanzo_body_for_hashing);

        // Convert the sorted JSON Value back to a string
        let hanzo_body_string = sorted_json.to_string();

        Ok(hanzo_body_string)
    }
...
mod tests {
    use crate::{
        hanzo_message::hanzo_message::{EncryptedHanzoBody, ExternalMetadata, HanzoVersion}, hanzo_utils::{encryption::EncryptionMethod, signatures::unsafe_deterministic_signature_keypair}
    };
    ...
    fn test_generate_desktop_installation_proof() {
        ...
        let result = HanzoMessage::generate_desktop_installation_proof(&public_key, &secret_key);
    ...
    fn test_calculate_message_hash_with_empty_outer_signature() {
        // Create a HanzoMessage instance with the provided data
        let message = HanzoMessage {
            body: MessageBody::Encrypted(EncryptedHanzoBody {
            ...
            external_metadata: ExternalMetadata {
                sender: "@@my_local_ai.sep-hanzo".to_string(),
                recipient: "@@my_local_ai.sep-hanzo".to_string(),
