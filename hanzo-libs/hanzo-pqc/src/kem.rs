...
//!   * `lux-crypto-impl` — canonical luxcpp/crypto FFI (`lux_crypto::mlkem`)
//!   * `legacy-oqs`      — Open Quantum Safe (`oqs::kem::Kem`)
//!
//! Default builds use `legacy-oqs` because the canonical luxcpp ML-KEM C-ABI
//! presently returns `CRYPTO_ERR_NOTIMPL`. Flip when impls land.
...
/// ML-KEM implementation. Backend chosen at compile time:
///   * `lux-crypto-impl` → routes through `lux_crypto::mlkem`
///   * `legacy-oqs`      → routes through `oqs::kem::Kem`
#[cfg(feature = "ml-kem")]
pub struct MlKem {
...
#[cfg(all(feature = "ml-kem", feature = "lux-crypto-impl"))]
#[async_trait]
impl Kem for MlKem {
    async fn generate_keypair(&self, alg: KemAlgorithm) -> Result<KemKeyPair> {
        ...
        let mode = alg.mlkem_mode()
            .ok_or_else(|| PqcError::UnsupportedAlgorithm(format!("{alg:?} not an ML-KEM variant")))?;
        let mut seed = [0u8; 32];
        getrandom::getrandom(&mut seed).map_err(|e| PqcError::RngError(e.to_string()))?;
        let (pk, sk) = lux_crypto::mlkem::keygen(mode, &seed)
            .map_err(|e| PqcError::KemError(format!("{e:?}")))?;
        Ok(KemKeyPair {
            encap_key: EncapsulationKey { algorithm: alg, key_bytes: pk },
            decap_key: DecapsulationKey { algorithm: alg, key_bytes: sk },
        })
    }

    async fn encapsulate(&self, encap_key: &EncapsulationKey) -> Result<KemOutput> {
        let mode = encap_key.algorithm.mlkem_mode()
            .ok_or_else(|| PqcError::UnsupportedAlgorithm(format!("{:?} not an ML-KEM variant", encap_key.algorithm)))?;
        let (ct, ss) = lux_crypto::mlkem::encap(mode, &encap_key.key_bytes)
            .map_err(|e| PqcError::KemError(format!("{e:?}")))?;
        Ok(KemOutput { ciphertext: ct, shared_secret: ss })
    }
    ...
    async fn decapsulate(
        ...
        let mode = decap_key.algorithm.mlkem_mode()
            .ok_or_else(|| PqcError::UnsupportedAlgorithm(format!("{:?} not an ML-KEM variant", decap_key.algorithm)))?;
        lux_crypto::mlkem::decap(mode, &decap_key.key_bytes, ciphertext)
            .map_err(|e| PqcError::KemError(format!("{e:?}")))
    }
}

#[cfg(all(feature = "ml-kem", feature = "legacy-oqs", not(feature = "lux-crypto-impl")))]
#[async_trait]
impl Kem for MlKem {
    async fn generate_keypair(&self, alg: KemAlgorithm) -> Result<KemKeyPair> {
