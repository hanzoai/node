...
mod tests {
    ...
    fn test_job_message() {
        ...
        let node_sender = "did:hanzo:localhost".to_string();
        let node_receiver = "did:hanzo:localhost".to_string();
        ...
        let message_result = HanzoMessageBuilder::job_message(
            inbox.clone(),
    ...
    fn test_builder_with_all_fields_no_encryption() {
        ...
        let recipient = "did:hanzo:other_node".to_string();
        let sender = "did:hanzo:my_node".to_string();
        ...
        if let MessageBody::Unencrypted(hanzo_body) = message.body {
            if let MessageData::Unencrypted(hanzo_data) = hanzo_body.message_data {
                assert_eq!(hanzo_data.message_raw_content, "body content");
            }
            assert_eq!(hanzo_body.internal_metadata.sender_subidentity, "");
            assert_eq!(hanzo_body.internal_metadata.recipient_subidentity, "");
            assert_eq!(
                hanzo_body.internal_metadata.inbox,
                "inbox::did:hanzo:my_node::did:hanzo:other_node::false"
            );
        }
    ...
    fn test_builder_with_all_fields_body_encryption() {
        ...
        if let MessageBody::Unencrypted(hanzo_body) = binding {
            let message_data = hanzo_body.message_data;
            if let MessageData::Unencrypted(hanzo_data) = message_data {
                let decrypted_content = hanzo_data.message_raw_content;
        ...
        let binding = decrypted_message.body.clone();
        if let MessageBody::Unencrypted(hanzo_body) = binding {
            let internal_metadata = &hanzo_body.internal_metadata;
    ...
    fn test_builder_with_all_fields_content_encryption() {
        ...
        if let MessageBody::Unencrypted(hanzo_body) = message.clone().body {
            assert_eq!(
                hanzo_body.internal_metadata.encryption,
                EncryptionMethod::DiffieHellmanChaChaPoly1305
            );
        }
        ...
        if let MessageBody::Unencrypted(hanzo_body) = decrypted_message.body {
            if let MessageData::Unencrypted(hanzo_data) = hanzo_body.message_data {
                assert_eq!(hanzo_data.message_raw_content, "body content");
            }
        }
    ...
    fn test_builder_use_code_registration() {
        ...
        let message_result = HanzoMessageBuilder::use_code_registration_for_device(
            my_encryption_sk.clone(),
            my_identity_sk,
        ...
        if let MessageBody::Unencrypted(hanzo_body) = decrypted_message.body {
            if let MessageData::Unencrypted(hanzo_data) = hanzo_body.message_data {
                ...
                let parsed_content: Result<RegistrationCodeSimple, _> =
                    serde_json::from_str(&hanzo_data.message_raw_content);
    ...
    fn test_initial_registration_with_no_code_for_device() {
        ...
        let message_result = HanzoMessageBuilder::initial_registration_with_no_code_for_device(
