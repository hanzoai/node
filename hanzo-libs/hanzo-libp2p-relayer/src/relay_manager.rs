...
pub struct RelayManager {
    ...
    request_response_channels: DashMap<request_response::OutboundRequestId, request_response::ResponseChannel<HanzoMessage>>, // request_id -> channel
    external_ip: Option<std::net::IpAddr>, // Store detected external IP
    registry: HanzoRegistry, // Blockchain registry for identity verification
...
impl RelayManager {
    ...
    async fn request_tool_offerings(&mut self, peer_id: PeerId) {
        if let Some(identity) = self.find_identity_by_peer(&peer_id) {
            ...
            let node_name = if let Ok(parsed_name) = hanzo_message_primitives::schemas::hanzo_name::HanzoName::new(identity.clone()) {
            ...
            let node_encryption_public_key = match self.registry.get_identity_record(node_name.clone(), None).await {
                Ok(identity_record) => {
                    match hanzo_message_primitives::hanzo_utils::encryption::string_to_encryption_public_key(&identity_record.encryption_key) {
                        Ok(key) => key,
                        Err(e) => {
            ...
            let request_message = match HanzoMessageBuilder::new(
    ...
    async fn handle_swarm_event(
        ...
        match event {
            ...
            SwarmEvent::Behaviour(RelayBehaviourEvent::RequestResponse(req_resp_event)) => {
                // Handle request-response events for relaying direct messages between Hanzo nodes
                    ...
                    request_response::Event::Message { peer, message, .. } => {
                        ...
                        match message {
                            request_response::Message::Request { mut request, channel, .. } => {
                                ...
                                let target_node = if let Ok(parsed_name) = hanzo_message_primitives::schemas::hanzo_name::HanzoName::new(target_identity.clone()) {
                                    parsed_name.get_node_name_string()
                                } else {
    ...
    async fn process_relay_message(&mut self, request: HanzoMessage, sender_peer: PeerId) -> Option<HanzoMessage> {
        ...
        let sender_enc_key = if sender_identity.contains("localhost") {
            ...
            match hanzo_message_primitives::hanzo_utils::encryption::string_to_encryption_public_key(&request.external_metadata.other) {
                Ok(key) => key,
                Err(e) => {
    ...
    fn create_ack_response(
        &self,
        original_request: &HanzoMessage,
        sender_encryption_key: &x25519_dalek::PublicKey,
        _processed_message: &HanzoMessage,
    ) -> Option<HanzoMessage> {
        // Create a simple ACK message using the HanzoMessageBuilder
        match HanzoMessageBuilder::ack_message(
            self.config.encryption_secret_key.clone(),
            self.config.identity_secret_key.clone(),
    ...
    async fn process_relay_response(&mut self, response: HanzoMessage, sender_peer: PeerId) {
        ...
        let decrypted_response = if response.encryption != hanzo_message_primitives::hanzo_utils::encryption::EncryptionMethod::None {
            // Get sender's encryption key for decryption
            let sender_enc_key = if sender_identity.contains("localhost") {
                println!("🔑 Relay: Using other field for localhost sender");
                match hanzo_message_primitives::hanzo_utils::encryption::string_to_encryption_public_key(&response.external_metadata.other) {
                    Ok(key) => key,
                    Err(e) => {
                        println!("❌ Relay: Failed to parse sender's encryption key from other field: {}", e);
                        return;
...
async fn relay_message_encryption(&mut self, request: &mut HanzoMessage, target_node: &String) {
        ...
        let recipient_name = match hanzo_message_primitives::schemas::hanzo_name::HanzoName::new(target_node.clone()) {
            Ok(name) => name,
            Err(_) => {
        ...
        let recipient_enc_key = if target_node.contains("localhost") {
            ...
            // Parse recipient's encryption key
            match hanzo_message_primitives::hanzo_utils::encryption::string_to_encryption_public_key(&recipient_identity.encryption_key) {
                Ok(key) => key,
                Err(_) => {
                    println!("❌ Relay: Failed to parse recipient's encryption key");
                    return;
        ...
        let original_sender_enc_key = match hanzo_message_primitives::hanzo_utils::encryption::string_to_encryption_public_key(&original_sender_identity.encryption_key) {
            Ok(key) => key,
            Err(_) => {
