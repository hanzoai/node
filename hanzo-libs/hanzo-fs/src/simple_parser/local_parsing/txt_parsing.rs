...
use crate::{hanzo_fs_error::HanzoFsError, simple_parser::{file_parser_helper::HanzoFileParser, text_group::TextGroup}};

impl LocalFileParser {
    /// Attempts to process the provided json file into a list of TextGroups.
    pub fn process_txt_file(file_buffer: Vec<u8>, max_node_text_size: u64) -> Result<Vec<TextGroup>, HanzoFsError> {
        let txt_string = String::from_utf8(file_buffer).map_err(|_| HanzoFsError::FailedTXTParsing)?;
            ...
            if (parsed_line.len() as u64 + current_text.len() as u64) > max_node_text_size {
                ...
                if parsed_line.len() as u64 > max_node_text_size {
                    ...
                    let chunks = if parsed_any_metadata {
                        HanzoFileParser::split_into_chunks_with_metadata(
                            &line,
    ...
    pub fn process_into_sentences(text: String) -> Vec<String> {
        ...
        text.split('\n')
            ...
            .flat_map(|line| {
                ...
                let re = Regex::new(HanzoFileParser::PURE_METADATA_REGEX).unwrap();
