use crate::{
    hanzo_fs_error::HanzoFsError, simple_parser::{file_parser_helper::HanzoFileParser, text_group::TextGroup}
};
...
impl LocalFileParser {
    ...
    pub fn process_csv_file(file_buffer: Vec<u8>, max_node_text_size: u64) -> Result<Vec<TextGroup>, HanzoFsError> {
        let csv_lines = Self::parse_csv_auto(&file_buffer).map_err(|_| HanzoFsError::FailedCSVParsing)?;
        Self::process_table_rows(csv_lines, max_node_text_size)
    }

    // /// Parse CSV data from a buffer and attempt to automatically detect
    // /// headers.
    pub fn parse_csv_auto(buffer: &[u8]) -> Result<Vec<String>, HanzoFsError> {
        let mut reader = ReaderBuilder::new().flexible(true).from_reader(Cursor::new(buffer));
        let headers = reader
            .headers()
            .map_err(|_| HanzoFsError::FailedCSVParsing)?
            .iter()
            .map(String::from)
            .collect::<Vec<String>>();
    ...
    // ///   values.
    pub fn parse_csv(buffer: &[u8], header: bool) -> Result<Vec<String>, HanzoFsError> {
        ...
        for record in reader.records() {
            let record = record.map_err(|_| HanzoFsError::FailedCSVParsing)?;
    ...
    pub fn process_table_rows(
        ...
        for line in joined_lines {
            let (parsed_line, metadata, parsed_any_metadata) = HanzoFileParser::parse_and_extract_metadata(&line);
            ...
            if parsed_line.len() as u64 > max_node_text_size {
                ...
                let chunks = if parsed_any_metadata {
                    HanzoFileParser::split_into_chunks_with_metadata(&line, max_node_text_size as usize)
                } else {
                    HanzoFileParser::split_into_chunks(&line, max_node_text_size as usize)
                };

                for chunk in chunks {
                    let (parsed_chunk, chunk_metadata, _) = if parsed_any_metadata {
                        HanzoFileParser::parse_and_extract_metadata(&chunk)
                    } else {
