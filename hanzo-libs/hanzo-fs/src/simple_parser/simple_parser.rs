...
impl SimpleParser {
    pub async fn parse_file(filepath: HanzoPath, max_node_text_size: u64) -> Result<Vec<TextGroup>, HanzoFsError> {
        // check if file exists
        if !filepath.exists() {
            return Err(HanzoFsError::FileNotFoundWithPath(filepath.to_string()));
        }
        ...
        if extension.is_none() {
            return Err(HanzoFsError::UnsupportedFileType(filepath.to_string()));
        }

        // check if the file extension is supported
        let file_type = SupportedFileType::from_extension(extension.unwrap())
            .ok_or_else(|| HanzoFsError::UnsupportedFileType(filepath.to_string()))?;

        // read file into memory
        let file_buffer = fs::read(&filepath.as_path()).map_err(|e| HanzoFsError::FailedIO(e.to_string()))?;

        // call the new function based on the file extension
        let absolute_path = path::absolute(filepath.as_path()).map_err(|e| HanzoFsError::FailedIO(e.to_string()))?;
        let text_groups =
    ...
    async fn process_file_by_extension(
        ...
        match file_type {
            ...
            SupportedFileType::Docx => LocalFileParser::process_docx_file(file_path, max_node_text_size).await,
...
mod tests {
    ...
    async fn test_parse_csv_file() {
        ...
        let hanzo_path = HanzoPath::from_string("test.csv".to_string());

        // Write a simple CSV content to the file
        let mut file = fs::File::create(&hanzo_path.as_path()).unwrap();
        writeln!(file, "header1,header2").unwrap();
        writeln!(file, "value1,value2").unwrap();

        // Call the parse_file function
        let result = SimpleParser::parse_file(hanzo_path, 1024).await;
    ...
    async fn test_parse_large_csv_file() {
        ...
        let hanzo_path = HanzoPath::from_string("large_test.csv".to_string());
        ...
        let result = SimpleParser::parse_file(hanzo_path, 20).await;
