...
impl HanzoFileManager {
    ...
    pub async fn save_and_process_file_with_embeddings(
        ...
        let parsed_file_id = sqlite_manager
            ...
            .ok_or(HanzoFsError::FailedToRetrieveParsedFileID)?
            .id
            .unwrap();
    ...
    pub fn get_absolute_path_for_additional_files(
        files: Vec<HanzoPath>,
        folders: Vec<HanzoPath>,
    ) -> Result<Vec<String>, HanzoFsError> {
    ...
    pub fn get_absolute_path_for_job_scope(
        ...
        let base_path = HanzoPath::base_path();
        let files = all_files
            .into_iter()
            .map(|file| {
        ...
        let files = files
            ...
            .map(|hanzo_path| {
                let file_info = FileInfo {
                    path: hanzo_path.relative_path().to_string(),
                    is_directory: false,
                    created_time: None,
                    modified_time: None,
                    ...
                    children: None,
                    size: None,
                    name: hanzo_path.filename().unwrap_or_default().to_string(),
                };
                return file_info;
            })
    ...
    pub fn get_all_files_and_folders_for_job(
        job_id: &str,
        sqlite_manager: &SqliteManager,
    ) -> Result<Vec<FileInfo>, HanzoFsError> {
    ...
    fn gather_directory_contents(
            ...
            .filter_map(|e| e.ok())
        {
            ...
            let hanzo_path = HanzoPath::new(&format!("{}/{}", rel_path, file_name));
    ...
    pub fn construct_job_file_path(
        ...
        // Create a new HanzoPath from the relative path
        Ok(HanzoPath::from_string(file_relative_path))
    }
    ...
    pub async fn save_and_process_file_with_jobid(
        ...
        let hanzo_path = Self::construct_job_file_path(job_id, &file_name, sqlite_manager)?;

        // Use the existing save_and_process_file function to save and process the file
        Self::save_and_process_file(hanzo_path.clone(), data, sqlite_manager, mode, generator).await?;

        Ok(hanzo_path)
    }

    /// Get the content of a file based on a HanzoPath.
    pub fn get_file_content(path: HanzoPath) -> Result<Vec<u8>, HanzoFsError> {
        let content = fs::read(path.as_path())
            .map_err(|_| HanzoFsError::FailedToReadFile(path.as_path().to_string_lossy().to_string()))?;
        Ok(content)
    }
    ...
    pub fn get_text_groups_with_embeddings(
        path: HanzoPath,
        sqlite_manager: &SqliteManager,
    ) -> Result<Vec<(String, Vec<f32>)>, HanzoFsError> {
        ...
        let parsed_file_id = parsed_file.id.ok_or(HanzoFsError::FailedToRetrieveParsedFileID)?;
        ...
        for chunk in chunks {
            let chunk_id = chunk.chunk_id.ok_or(HanzoFsError::FailedToRetrieveParsedFileID)?;
...
mod tests {
    ...
    fn setup_test_environment() -> (SqliteManager, tempfile::TempDir, HanzoPath, MockGenerator) {
        ...
        let vr_path = HanzoPath::from_base_path();
    ...
    fn test_list_directory_contents() {
        let (db, _dir, _hanzo_path, _generator) = setup_test_environment();

        // Create a temporary directory
        let dir_path = HanzoPath::from_base_path();

        // Create a subdirectory and a file inside the temporary directory
        let subdir_path = HanzoPath::from_string("subdir".to_string());
        ...
        writeln!(file, "Hello, Hanzo!").unwrap();

        // Call the function to list directory contents
        let contents = HanzoFileManager::list_directory_contents(dir_path, &db).unwrap();
    ...
    fn test_list_directory_contents_with_db_entries() {
        ...
        let pf1_path = HanzoPath::from_string("january.txt".to_string());
        let pf2_path = HanzoPath::from_string("february.txt".to_string());
        ...
        let pf3_path = HanzoPath::from_string("march.txt".to_string());
    ...
    async fn test_process_file() {
        let (db, dir, hanzo_path, generator) = setup_test_environment();

        // Create and write to the file
        let mut file = File::create(hanzo_path.as_path()).unwrap();
        write_large_content(&mut file);

        // Call the process_embeddings_for_file function
        let result = HanzoFileManager::process_embeddings_for_file(
            hanzo_path.clone(),
            &db,
    ...
    async fn test_save_and_process_file() {
        ...
        let mut file = File::create(hanzo_path.as_path()).unwrap();
        write_large_content(&mut file);
        let data = std::fs::read(hanzo_path.as_path()).unwrap();

        // Call the save_and_process_file function
        let result = HanzoFileManager::save_and_process_file(
            hanzo_path.clone(),
            data,
            &db,
            FileProcessingMode::Auto,
    ...
    async fn test_create_job_and_upload_file() {
        let (db, _dir, _hanzo_path, generator) = setup_test_environment();
        ...
        let file_content = b"Hello, Hanzo!".to_vec();
        let file_path = folder_path.as_path().join(file_name);
        let file_path = HanzoPath::from_string(file_path.to_string_lossy().to_string());
        ...
        let contents = HanzoFileManager::list_directory_contents(folder_path, &db).unwrap();
    ...
    fn test_get_file_content() {
        let (_db, _dir, _hanzo_path, _generator) = setup_test_environment();

        // Create a specific file path within the temporary directory
        let file_name = "test_file.txt";
        let file_path = HanzoPath::from_string(file_name.to_string());
        ...
        let content = HanzoFileManager::get_file_content(file_path.clone());

        // Assert the content is as expected
        assert!(content.is_ok());
        assert_eq!(content.unwrap(), b"Hello, Hanzo!\n".to_vec());
    }
    ...
    fn test_construct_job_file_path() {
        ...
        let file_name = "test_file.txt";
        let result = HanzoFileManager::construct_job_file_path(job_id, file_name, &db);
        ...
        assert_eq!(
            hanzo_path.as_path().to_string_lossy(),
    ...
    fn test_list_directory_contents_with_depth() {
        ...
        let base_dir = HanzoPath::from_base_path();
        ...
        let pf1 = create_test_parsed_file(
            1,
            &HanzoPath::from_string("level1/file1.txt".to_string()).relative_path(),
        );
        let pf2 = create_test_parsed_file(
            2,
            &HanzoPath::from_string("level1/level2/file2.txt".to_string()).relative_path(),
        );
        db.add_parsed_file(&pf1).unwrap();
        db.add_parsed_file(&pf2).unwrap();

        // Call the function to list directory contents with depth 3
        let contents = HanzoFileManager::list_directory_contents_with_depth(base_dir, &db, 3).unwrap();
        eprintln!("contents: {:?}", contents);
    ...
    fn test_search_files_by_content() {
        ...
        let results = HanzoFileManager::search_files_by_content(base_path.clone(), "January 2024", &db).unwrap();
        ...
        let results = HanzoFileManager::search_files_by_content(base_path.clone(), "2024", &db).unwrap();
        assert_eq!(results.len(), 5); // Should find all files as they all contain "2024"

        // Test case insensitive match
        let results = HanzoFileManager::search_files_by_content(base_path.clone(), "REPORT", &db).unwrap();
        assert_eq!(results.len(), 2); // Should find files containing "report" regardless of case

        // Test content spanning multiple files
        let results = HanzoFileManager::search_files_by_content(base_path.clone(), "meeting", &db).unwrap();
        assert_eq!(results.len(), 2); // Should find both files containing "meeting"

        // Test no matches
        let results = HanzoFileManager::search_files_by_content(base_path.clone(), "nonexistent", &db).unwrap();
    ...
    fn test_search_files_by_name() {
        ...
        let results = HanzoFileManager::search_files_by_name(base_path.clone(), "january.txt", &db).unwrap();
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].path, os_path::OsPath::from("docs/reports/2024/january.txt").to_string());
        assert!(results[0].has_embeddings);

        // Test partial name match
        let results = HanzoFileManager::search_files_by_name(base_path.clone(), "2024", &db).unwrap();
        assert_eq!(results.len(), 4); // Should find all files whose name contains "2024"

        // Test case insensitive match
        let results = HanzoFileManager::search_files_by_name(base_path.clone(), "REPORT", &db).unwrap();
        assert_eq!(results.len(), 3); // Should find files containing "report" regardless of case

        // Test name spanning multiple files
        let results = HanzoFileManager::search_files_by_name(base_path.clone(), "meeting", &db).unwrap();
        assert_eq!(results.len(), 0); // Should not find any files containing "meeting" because is searching now only by name

        // Test no matches
        let results = HanzoFileManager::search_files_by_name(base_path.clone(), "nonexistent", &db).unwrap();
        assert_eq!(results.len(), 0);
    ...
    fn test_search_files_by_name_and_content() {
        ...
        let name_results = HanzoFileManager::search_files_by_name(base_path.clone(), "january.txt", &db).unwrap();
        ...
        let content_results =
            HanzoFileManager::search_files_by_content(base_path.clone(), "monthly report", &db).unwrap();
        ...
        let combined_results =
            HanzoFileManager::search_files_by_name_and_content(base_path.clone(), "january.txt", &db).unwrap();
        ...
        let combined_results =
            HanzoFileManager::search_files_by_name_and_content(base_path.clone(), "quarterly update", &db).unwrap();
        assert_eq!(combined_results.len(), 1);
        ...
        let combined_results =
            HanzoFileManager::search_files_by_name_and_content(base_path.clone(), "2024", &db).unwrap();
        ...
        let combined_results =
            HanzoFileManager::search_files_by_name_and_content(base_path.clone(), "meeting", &db).unwrap();
    ...
    async fn test_save_process_and_retrieve_embeddings() {
        ...
        HanzoFileManager::write_file_to_fs(hanzo_path.clone(), data.clone()).unwrap();
        ...
        let result = HanzoFileManager::save_and_process_file_with_embeddings(
            hanzo_path.clone(),
            data,
            &db,
        ...
        let parsed_file = db.get_parsed_file_by_rel_path(&hanzo_path.relative_path()).unwrap();
        ...
        let chunks: Vec<HanzoFileChunk> = db.get_chunks_for_parsed_file(parsed_file_id).unwrap();
        ...
        let text_groups_with_embeddings =
            HanzoFileManager::get_text_groups_with_embeddings(hanzo_path.clone(), &db).unwrap();
        assert!(
