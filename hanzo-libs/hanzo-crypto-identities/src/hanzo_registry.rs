...
use hanzo_message_primitives::hanzo_utils::hanzo_logging::hanzo_log;
use hanzo_message_primitives::hanzo_utils::hanzo_logging::HanzoLogLevel;
use hanzo_message_primitives::hanzo_utils::hanzo_logging::HanzoLogOption;
use hanzo_message_primitives::hanzo_utils::signatures::string_to_signature_public_key;
use hanzo_non_rust_code::functions::get_identity_data::get_identity_data;
use std::fmt;
use std::fs;
...
impl From<AddrParseError> for HanzoRegistryError {
    fn from(err: AddrParseError) -> HanzoRegistryError {
        HanzoRegistryError::AddressParseError(err)
    }
}

impl From<std::time::SystemTimeError> for HanzoRegistryError {
    fn from(err: std::time::SystemTimeError) -> HanzoRegistryError {
        HanzoRegistryError::SystemTimeError(err)
    }
}

impl From<serde_json::Error> for HanzoRegistryError {
    fn from(err: serde_json::Error) -> HanzoRegistryError {
        HanzoRegistryError::JsonError(err)
    }
}

impl From<std::io::Error> for HanzoRegistryError {
    fn from(err: std::io::Error) -> HanzoRegistryError {
        HanzoRegistryError::IoError(err)
    }
}

impl std::error::Error for HanzoRegistryError {}

#[derive(Debug, PartialEq, Clone)]
pub struct OnchainIdentity {
    pub hanzo_identity: String,
...
impl OnchainIdentity {
    pub async fn first_address(&self) -> Result<SocketAddr, HanzoRegistryError> {
        hanzo_log(
            HanzoLogOption::CryptoIdentity,
            HanzoLogLevel::Info,
            format!(
                "Getting first address for identity: {} with addresses: {:?}",
                self.hanzo_identity, self.address_or_proxy_nodes
            )
            .as_str(),
        );
        ...
        match address.parse::<SocketAddr>() {
            ...
            Err(_) => {
                ...
                resolved_addresses
                    ...
                    .ok_or_else(|| HanzoRegistryError::CustomError("No address resolved".to_string()))
            }
        }
    }

    fn validate_address(first_address: &str) -> Result<String, HanzoRegistryError> {
    ...
    pub fn encryption_public_key(&self) -> Result<PublicKey, HanzoRegistryError> {
        string_to_encryption_public_key(&self.encryption_key)
            .map_err(|err| HanzoRegistryError::CustomError(err.to_string()))
    }

    pub fn signature_verifying_key(&self) -> Result<VerifyingKey, HanzoRegistryError> {
        string_to_signature_public_key(&self.signature_key)
            .map_err(|err| HanzoRegistryError::CustomError(err.to_string()))
    }
}

pub trait HanzoRegistryTrait {
    fn new(url: &str, contract_address: &str, abi_path: &str) -> Result<Self, HanzoRegistryError>
    where
        Self: Sized;
    fn get_identity_record(&self, identity: String) -> Result<OnchainIdentity, HanzoRegistryError>;
    fn get_cache_time(&self, identity: &str) -> Option<SystemTime>;
}

#[derive(Debug, Clone)]
pub struct HanzoRegistry {
...
impl HanzoRegistry {
    ...
    ) -> Result<Self, HanzoRegistryError> {
        let abi_file_content = match abi_path {
            Some(path) => fs::read_to_string(path).map_err(HanzoRegistryError::IoError)?,
            None => {
                hanzo_log(
                    HanzoLogOption::CryptoIdentity,
                    HanzoLogLevel::Info,
                ...
                include_str!("./abi/HanzoRegistry.sol/HanzoRegistry.json").to_string()
            }
        };
    ...
    pub async fn fetch_identity_record(
        ...
        let identity_data = get_identity_data(rpc_endpoints.clone(), contract_address.clone(), contract_abi.clone(), identity.clone())
            .await
            .map_err(|e| HanzoRegistryError::IdentityFetchError(e.to_string()))?
            .identity_data;

        if identity_data.is_none() {
            return Err(HanzoRegistryError::IdentityNotFound(format!(
                "identity '{}' not found",
                identity.clone()
            )));
        }
