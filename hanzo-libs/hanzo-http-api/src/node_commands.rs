...
use hanzo_message_primitives::{
    schemas::{
        coinbase_mpc_config::CoinbaseMPCWalletConfig, crontab::{CronTask, CronTaskAction}, custom_prompt::CustomPrompt, identity::{Identity, StandardIdentity}, job_config::JobConfig, llm_providers::{agent::Agent, serialized_llm_provider::SerializedLLMProvider, hanzo_backend::QuotaResponse}, mcp_server::MCPServer, hanzo_name::HanzoName, hanzo_tool_offering::{HanzoToolOffering, UsageTypeInquiry}, hanzo_tools::{CodeLanguage, DynamicToolType}, smart_inbox::{SmartInbox, V2SmartInbox}, tool_router_key::ToolRouterKey, wallet_complementary::{WalletRole, WalletSource}, wallet_mixed::NetworkIdentifier, x402_types::Network
    }, hanzo_message::{
        hanzo_message::HanzoMessage, hanzo_message_schemas::{
...
use hanzo_tools_primitives::tools::{
    mcp_server_tool::MCPServerTool, hanzo_tool::{HanzoTool, HanzoToolHeader, HanzoToolWithAssets}, tool_config::OAuth, tool_playground::ToolPlayground, tool_types::{OperatingSystem, RunnerType}
};
// use crate::{
//     prompts::custom_prompt::CustomPrompt, tools::hanzo_tool::{HanzoTool, HanzoToolHeader}, wallet::{
...
pub enum NodeCommand {
    ...
    // Command to make the node send a `HanzoMessage` in an onionized (i.e., anonymous and encrypted) way.
    ...
    // Command to make the node use a registration code encapsulated in a `HanzoMessage`. The sender will receive
    // the result.
    ...
    GetLastMessagesFromInboxWithBranches {
        ...
        limit: usize,
        offset_key: Option<String>,
        res: Sender<Vec<Vec<HanzoMessage>>>,
    },
    APICreateJob {
        msg: HanzoMessage,
        res: Sender<Result<String, APIError>>,
    ...
    V2ApiSearchHanzoTool {
        bearer: String,
        query: String,
        agent_or_llm: Option<String>,
        res: Sender<Result<Value, APIError>>,
    },
    V2ApiListAllHanzoTools {
        bearer: String,
        category: Option<String>,
        res: Sender<Result<Value, APIError>>,
    },
    V2ApiListAllMcpHanzoTools {
        category: Option<String>,
        res: Sender<Result<Value, APIError>>,
    },
    V2ApiListAllNetworkHanzoTools {
        bearer: String,
        res: Sender<Result<Value, APIError>>,
    },
    V2ApiListAllHanzoToolsVersions {
        bearer: String,
        res: Sender<Result<Value, APIError>>,
    },
    V2ApiSetHanzoTool {
    ...
    V2ApiGetToolOffering {
        ...
        res: Sender<Result<HanzoToolOffering, APIError>>,
    },
    ...
    V2ApiGetAllToolOfferings {
        bearer: String,
        res: Sender<Result<Vec<HanzoToolHeader>, APIError>>,
    },
    V2ApiSetToolOffering {
        bearer: String,
        tool_offering: HanzoToolOffering,
        res: Sender<Result<HanzoToolOffering, APIError>>,
    },
