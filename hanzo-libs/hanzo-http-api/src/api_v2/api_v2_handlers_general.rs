...
use hanzo_message_primitives::schemas::llm_providers::serialized_llm_provider::{
    Exo, Gemini, Groq, LLMProviderInterface, Ollama, OpenAI, HanzoBackend
};
...
use hanzo_message_primitives::hanzo_message::hanzo_message::{
    EncryptedHanzoBody, EncryptedHanzoData, ExternalMetadata, InternalMetadata, MessageBody, MessageData, NodeApiData, HanzoBody, HanzoData, HanzoMessage, HanzoVersion
};
...
use hanzo_message_primitives::{
    schemas::llm_providers::serialized_llm_provider::SerializedLLMProvider, hanzo_message::hanzo_message_schemas::APIAddOllamaModels
};
...
pub fn general_routes(
    ...
    let hanzo_backend_quota_route = warp::path("hanzo_backend_quota")
        .and(warp::get())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        ...
        .and_then(|sender, auth, params: HashMap<String, String>| {
            ...
            hanzo_backend_quota_handler(sender, auth, model_type)
        });
...
pub async fn hanzo_backend_quota_handler(
    ...
    let (res_sender, res_receiver) = async_channel::bounded(1);
    sender
        .send(NodeCommand::V2ApiHanzoBackendGetQuota {
            bearer,
            model_type,
            res: res_sender,
        })
...
#[openapi(
    ...
    components(
        schemas(APIAddOllamaModels, SerializedLLMProvider, HanzoName, LLMProviderInterface,
            HanzoMessage, MessageBody, EncryptionMethod, ExternalMetadata, HanzoVersion,
            OpenAI, Ollama, Groq, Gemini, Exo, EncryptedHanzoBody, HanzoBody,
            HanzoSubidentityType, HanzoBackend, InternalMetadata, MessageData, StopLLMRequest,
            NodeApiData, EncryptedHanzoData, HanzoData, MessageSchemaType,
