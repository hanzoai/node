...
pub fn wallet_routes(
    ...
    let node_wallet_route = warp::path("node_wallet")
        .and(warp::get())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization").or(warp::any().map(String::new)).unify())
        .and_then(node_wallet_handler);
...
#[utoipa::path(
    post,
    path = "/v1/node/restore_local_wallet",
...
#[utoipa::path(
    post,
    path = "/v1/node/create_local_wallet",
...
#[utoipa::path(
    post,
    path = "/v1/node/restore_coinbase_mpc_wallet",
...
#[utoipa::path(
    get,
    path = "/v1/node/get_wallet_balance",
    responses(
...
#[utoipa::path(
    ...
    path = "/v2/node_wallet",
    responses(
        (status = 200, description = "Node's unified wallet/identity (EVM address, secp256k1 private key, DID)", body = Value),
        (status = 500, description = "Internal server error", body = APIError)
    )
)]
pub async fn node_wallet_handler(
    sender: Sender<NodeCommand>,
    authorization: String,
) -> Result<impl warp::Reply, warp::Rejection> {
    let bearer = authorization.strip_prefix("Bearer ").unwrap_or("").to_string();
    let (res_sender, res_receiver) = async_channel::bounded(1);
    sender
        .send(NodeCommand::V2ApiNodeWallet {
            bearer,
            res: res_sender,
        })
        .await
