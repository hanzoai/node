            ...
            */
            ...
            // Check the endpoints that return all the branches
            {
                ...
                loop {
                    ...
                    // Assuming each HanzoMessage can be uniquely identified or grouped by its content
                    // and that you have a way to determine the structure [[1],[2],[5,3],[6]] from these messages

                    // Since node1_last_messages is Vec<Vec<HanzoMessage>>, we need to iterate through both levels
                    let flattened_messages = node1_last_messages
                        ...
                        .flat_map(|msg_group| {
                            msg_group
                                .iter()
                                .map(|msg| {
                                    // Assuming get_message_content() is a method of HanzoMessage
                                    msg.get_message_content().unwrap() // Apply get_message_content to each
                                                                       // HanzoMessage
                                })
                                .collect::<Vec<_>>() // Collects all message contents in the inner Vec<HanzoMessage>
                        })
                        .collect::<Vec<_>>(); // Collects all contents across all Vec<Vec<HanzoMessage>>
