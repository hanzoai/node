...
async fn create_local_node_profile(
    ...
    let node_profile = HanzoName::new(node_profile_name.clone()).unwrap();
...
async fn test_generate_and_use_registration_code_for_specific_profile() {
    ...
    create_local_node_profile(&hanzo_db, node_profile_name.to_string(), encryption_pk, identity_pk).await;
    ...
    let registration_code = hanzo_db
    ...
    // check in db
    let profile_name =
        HanzoName::from_node_and_profile_names(node_profile_name.to_string(), profile_name.to_string()).unwrap();
    let permission_in_db: IdentityPermissions = hanzo_db.get_profile_permission(profile_name).unwrap();
...
async fn test_generate_and_use_registration_code_for_device() {
    ...
    let _profile_result = hanzo_db.use_registration_code(
        &registration_code,
        node_profile_name,
        profile_name,
        &signature_public_key_to_string(profile_identity_pk),
    ...
    let profile_name =
        HanzoName::from_node_and_profile_names(node_profile_name.to_string(), profile_name.to_string()).unwrap();
    let permission_in_db = hanzo_db.get_profile_permission(profile_name.clone()).unwrap();
    assert_eq!(permission_in_db, IdentityPermissions::Admin);

    // check device permission
    let device_name = HanzoName::from_node_and_profile_names_and_type_and_name(
    ...
    .unwrap();
    let permission_in_db = hanzo_db.get_device_permission(device_name).unwrap();
...
async fn test_generate_and_use_registration_code_for_device_with_main_profile() {
    ...
    let _device_result = hanzo_db.use_registration_code(
        &registration_code,
        node_profile_name,
        device_name,
        &signature_public_key_to_string(profile_identity_pk),
    ...
    let main_profile_name =
        HanzoName::from_node_and_profile_names(node_profile_name.to_string(), "main".to_string()).unwrap();
    let main_permission_in_db = hanzo_db.get_profile_permission(main_profile_name.clone()).unwrap();
    assert_eq!(main_permission_in_db, IdentityPermissions::Admin);

    // Check if device exists in db
    let device_full_name = HanzoName::from_node_and_profile_names_and_type_and_name(
        node_profile_name.to_string(),
        "main".to_string(),
        HanzoSubidentityType::Device,
        device_name.to_string(),
    ...
    .unwrap();
    let device_permission_in_db = hanzo_db.get_device_permission(device_full_name.clone()).unwrap();
    ...
    let device_in_db = hanzo_db.get_device(device_full_name.clone()).unwrap();
...
async fn test_generate_and_use_registration_code_no_associated_profile() {
    ...
    // Test use_registration_code
    let result = hanzo_db.use_registration_code(
        &registration_code,
        node_profile_name,
        device_name,
...
async fn test_new_load_all_sub_identities() {
    ...
    for i in 1..=5 {
        ...
        let identity = StandardIdentity::new(
            HanzoName::from_node_and_profile_names(node_profile_name.to_string(), subidentity_name.to_string())
                .unwrap(),
            None,
            encryption_pk,
            identity_pk,
...
async fn test_new_insert_profile() {
    ...
    assert!(
        !hanzo_db.has_any_profile().unwrap(),
    ...
    let identity = StandardIdentity::new(
        HanzoName::from_node_and_profile_names(node_profile_name.to_string(), subidentity_name.to_string()).unwrap(),
        None,
        encryption_pk,
        identity_pk,
        Some(subencryption_pk),
    ...
    // Test insert_profile
    hanzo_db.insert_profile(identity.clone()).unwrap();
...
async fn test_remove_profile() {
    ...
    // Remove identity
    hanzo_db.remove_profile(subidentity_name).unwrap();
}
