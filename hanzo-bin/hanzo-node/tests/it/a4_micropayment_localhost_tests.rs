        ...
        "content": [
            {
                ...
                "sql_queries": [],
                "sql_tables": [],
                "tool_router_key": "local:::__localhost_sep_hanzo:::echo_function",
                "tool_set": "",
                "tools": [],
                "version": "1.0.0"
            },
...
#[test]
fn micropayment_localhost_flow_test() {
    #[cfg(feature = "console")]
    {
    ...
    let e = rt.block_on(async {
        let node1_identity_name = "@@node1_with_libp2p_relayer.sep-hanzo"; // The one that is running the tool
        let node2_identity_name = "@@localhost.sep-hanzo";  // The one that is requesting the invoice / tool
        let node1_profile_name = "main";
        let node1_device_name = "node1_device";
        let node2_profile_name = "main";
        let relay_identity_name = "@@libp2p_relayer.sep-hanzo";
        ...
        let interactions_handler = tokio::spawn(async move {
            ...
            let test_network_tool_name = "__node1_with_libp2p_relayer_sep_hanzo:::__localhost_sep_hanzo:::echo_function";
            let test_local_tool_key_name = "local:::__localhost_sep_hanzo:::echo_function";

            let hanzo_tool_offering = HanzoToolOffering {
            ...
            let hanzo_tool_header = HanzoToolHeader {
                ...
                config: Some(vec![]),
                usage_type: None,
                tool_offering: Some(hanzo_tool_offering.clone()),
            };

            {
                ...
                let hanzo_tool = HanzoTool::Deno(deno_tool, is_active);

                let (sender, receiver) = async_channel::bounded(1);
                node1_commands_sender
                    .send(NodeCommand::V2ApiAddHanzoTool {
                        bearer: api_v2_key.to_string(),
                        hanzo_tool: HanzoToolWithAssets {
                            tool: hanzo_tool,
                            assets: None,
                        },
                        res: sender,
                ...
                eprintln!("\nAll available Hanzo tools (name, tool_router_key):");
                ...
                // // Retrieve the hanzo_tool from node1
                // let (sender, receiver) = async_channel::bounded(1);
                // node1_commands_sender
                //     .send(NodeCommand::V2ApiGetHanzoTool {
                //         bearer: api_v2_key.to_string(),
                //         payload: "__node1_test_sep_hanzo:::__official_hanzo:::echo_function".to_string(),
                ...
                node1_commands_sender
                    .send(NodeCommand::V2ApiSetToolOffering {
                        bearer: api_v2_key.to_string(),
                        tool_offering: hanzo_tool_offering.clone(),
                        res: sender,
                    })
                    .await
                    .unwrap();
                ...
                if let Err(api_error) = &resp {
                    // Assuming api_error is of a type that has a `message` field (e.g., APIError from hanzo_http_api)
            ...
            }
            {
                ...
                let expected_response = vec![hanzo_tool_header.clone()];
            ...
            }
            {
                ...
                // Convert HanzoToolHeader to HanzoTool
                // Manually create NetworkTool
                let provider = HanzoName::new(node1_identity_name.to_string()).unwrap();
                let tool_router_key = ToolRouterKey::new(
                    provider.to_string(),
                    hanzo_tool_header.author.clone(),
                    hanzo_tool_header.name.clone(),
                    None,
                );
                let network_tool = NetworkTool {
                    name: hanzo_tool_header.name.clone(),
                    author: hanzo_tool_header.author.clone(),
                    description: hanzo_tool_header.description.clone(),
                    version: hanzo_tool_header.version.clone(),
                    mcp_enabled: hanzo_tool_header.mcp_enabled.clone(),
                    provider,
                    tool_router_key: tool_router_key.to_string_without_version(),
                    usage_type: hanzo_tool_offering.usage_type.clone(),
                    activated: hanzo_tool_header.enabled,
                    config: hanzo_tool_header.config.clone().unwrap_or_default(),
                ...
                let hanzo_tool = HanzoTool::Network(network_tool, true);

                let serialized_hanzo_tool = serde_json::to_value(&hanzo_tool).unwrap();
                eprintln!("serialized_hanzo_tool: {:?}", serialized_hanzo_tool);

                // Add the HanzoTool to node2
                let (sender, receiver) = async_channel::bounded(1);
                node2_commands_sender
                    .send(NodeCommand::V2ApiAddHanzoTool {
                        bearer: api_v2_key.to_string(),
                    ...
                    .unwrap();
                let resp = receiver.recv().await.unwrap();
                eprintln!("resp add hanzo tool to node2: {:?}", resp);
                ...
                node2_commands_sender
                    ...
                    .unwrap();
                let resp = receiver.recv().await.unwrap();
                eprintln!("\nAll available Hanzo tools in node2 (name, tool_router_key):");
            ...
            }
            {
                ...
                // Search for the tool using the V2ApiSearchHanzoTool command
                let (sender, receiver) = async_channel::bounded(1);
                node2_commands_sender
                    ...
                    .unwrap();
                let resp = receiver.recv().await.unwrap();
                eprintln!("resp search hanzo tool: {:?}", resp);
            ...
            }
            {
                ...
                node2_commands_sender
                    .send(NodeCommand::V2ApiPayInvoice {
                        ...
                        data_for_tool: serde_json::json!({ "message": "Hello, Hanzo!" }),
                        res: sender,
                    })
                    .await
                    .unwrap();
