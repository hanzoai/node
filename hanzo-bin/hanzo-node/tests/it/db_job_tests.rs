...
mod tests {
    ...
    use hanzo_message_primitives::{
        schemas::{
            identity::{StandardIdentity, StandardIdentityType}, inbox_name::InboxName, inbox_permission::InboxPermission, job::ForkedJob, hanzo_name::HanzoName, subprompts::SubPrompt
        }, hanzo_message::hanzo_message_schemas::{IdentityPermissions, JobMessage}, hanzo_utils::{
            encryption::unsafe_deterministic_encryption_keypair, job_scope::MinimalJobScope, hanzo_message_builder::HanzoMessageBuilder, signatures::{clone_signature_secret_key, unsafe_deterministic_signature_keypair}
        }
    };
    ...
    async fn test_create_new_job() {
        ...
        // Create a new job
        let _ = create_new_job(&hanzo_db, job_id.clone(), agent_id.clone(), scope).await;

        // Retrieve all jobs
        let jobs = hanzo_db.get_all_jobs().unwrap();
        ...
        let job = hanzo_db.get_job(&job_id).unwrap();
    ...
    async fn test_get_agent_jobs() {
        ...
        for i in 1..=5 {
            ...
            let scope = MinimalJobScope::default();
            let _ = create_new_job(&hanzo_db, job_id, agent_id.clone(), scope).await;
        }

        eprintln!("agent_id: {}", agent_id.clone());

        // Get all jobs for the agent
        let jobs = hanzo_db.get_agent_jobs(agent_id.clone()).unwrap();
    ...
    async fn test_change_job_agent() {
        ...
        let _ = create_new_job(&hanzo_db, job_id.clone(), initial_agent_id.clone(), scope).await;

        // Change the agent of the job
        hanzo_db.change_job_llm_provider(&job_id, &new_agent_id).unwrap();
        ...
        let new_agent_jobs = hanzo_db.get_agent_jobs(new_agent_id.clone()).unwrap();
        ...
        let initial_agent_jobs = hanzo_db.get_agent_jobs(initial_agent_id.clone()).unwrap();
    ...
    async fn test_update_job_to_finished() {
        ...
        hanzo_db.update_job_to_finished(&job_id.clone()).unwrap();

        // Retrieve the job and check that is_finished is set to true
        let job = hanzo_db.get_job(&job_id.clone()).unwrap();
        assert!(job.is_finished);
    }
    ...
    async fn test_get_agent_jobs_none_exist() {
        ...
        let jobs_result = hanzo_db.get_agent_jobs(agent_id.clone());
    ...
    async fn test_job_inbox_empty() {
        ...
        assert!(hanzo_db.is_job_inbox_empty(&job_id).unwrap());

        let (placeholder_signature_sk, _) = unsafe_deterministic_signature_keypair(0);
        let hanzo_message = HanzoMessageBuilder::job_message_from_llm_provider(
            job_id.to_string(),
            "something".to_string(),
            None,
            vec![],
        ...
        let _ = hanzo_db
            .add_message_to_job_inbox(&job_id.clone(), &hanzo_message, None, None)
            .await;
    ...
    async fn test_job_inbox_tree_structure() {
        ...
        for i in 1..=4 {
            ...
            // Add a message to the job
            let _ = hanzo_db
                .add_message_to_job_inbox(&job_id.clone(), &hanzo_message, parent_hash.clone(), None)
                .await;

            // Update the parent message according to the tree structure
            if i == 1 {
                parent_message_hash = Some(hanzo_message.calculate_message_hash_for_pagination());
            } else if i == 2 {
                parent_message_hash_2 = Some(hanzo_message.calculate_message_hash_for_pagination());
            }
        ...
        let last_messages_inbox = hanzo_db
    ...
    async fn test_insert_steps_with_simple_tree_structure() {
        ...
        let _ = create_new_job(&hanzo_db, job_id.to_string(), agent_id.clone(), scope).await;
        ...
        for i in 1..=4 {
            ...
            // Insert the HanzoMessage into the database
            hanzo_db
        ...
        let step_history = hanzo_db.get_step_history(job_id, true).unwrap().unwrap();

        let step_history_content: Vec<String> = step_history
            .iter()
            .map(|hanzo_message| {
                eprintln!("Hanzo message: {:?}", hanzo_message);
                let prompt = hanzo_message.to_prompt();
    ...
    async fn test_add_forked_job() {
        ...
        // Create a new job
        let _ = create_new_job(&hanzo_db, job_id.clone(), agent_id.clone(), scope.clone()).await;
        ...
        let forked_message2_id = last_messages_inbox
            ...
            .unwrap()
            .calculate_message_hash_for_pagination();
        let _ = create_new_job(&hanzo_db, forked_job1_id.clone(), agent_id.clone(), scope.clone()).await;
        let _ = create_new_job(&hanzo_db, forked_job2_id.clone(), agent_id.clone(), scope).await;
        ...
        match hanzo_db.add_forked_job(&job_id, forked_job1) {
            Ok(_) => {}
            Err(e) => panic!("Error adding forked job: {:?}", e),
        }
        match hanzo_db.add_forked_job(&job_id, forked_job2) {
            Ok(_) => {}
            Err(e) => panic!("Error adding forked job: {:?}", e),
        }
    ...
    async fn test_remove_job() {
        ...
        // Create new jobs
        let _ = create_new_job(&hanzo_db, job1_id.clone(), agent_id.clone(), scope.clone()).await;
        let _ = create_new_job(&hanzo_db, job2_id.clone(), agent_id.clone(), scope).await;
        ...
        let node1_profile_identity = StandardIdentity::new(
            HanzoName::from_node_and_profile_names(
                node1_identity_name.to_string(),
                node1_subidentity_name.to_string(),
            )
            .unwrap(),
        ...
        let _ = hanzo_db.insert_profile(node1_profile_identity.clone());
        ...
        hanzo_db.remove_job(&job1_id).unwrap();

        // Check if the job is removed
        match hanzo_db.get_job(&job1_id) {
