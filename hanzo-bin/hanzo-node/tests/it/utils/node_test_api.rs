...
pub async fn api_registration_device_node_profile_main(
...
) {
    {
        ...
        let code_message = HanzoMessageBuilder::use_code_registration_for_device(
...
pub async fn api_registration_profile_node(
...
) {
    {
        ...
        let msg = HanzoMessageBuilder::request_code_registration(
        ...
        let code_message = HanzoMessageBuilder::use_code_registration_for_profile(
            subidentity_encryption_sk.clone(),
            clone_signature_secret_key(&subidentity_signature_sk),
            node_encryption_pk,
        ...
        assert_eq!(
            ...
            HanzoName::from_node_and_profile_names(node_identity_name.to_string(), node_profile_name.to_string())
                .unwrap(),
            "Node has the right subidentity"
        );
    }
...
pub async fn api_llm_provider_registration(
    ...
    {
        let code_message = HanzoMessageBuilder::request_add_llm_provider(
            subidentity_encryption_sk.clone(),
            clone_signature_secret_key(&subidentity_signature_sk),
            node_encryption_pk,
            llm_provider.clone(),
        ...
        let agent_identity = node2_all_subidentities.iter().find(|identity| {
            identity.get_full_identity_name()
                == HanzoName::new(format!("{}/main/agent/{}", node_name, llm_provider.id))
                    .unwrap()
                    .to_string()
        });
        ...
        let available_llm_providers_msg = HanzoMessageBuilder::create_custom_hanzo_message_to_node(
            subidentity_encryption_sk.clone(),
            clone_signature_secret_key(&subidentity_signature_sk),
            node_encryption_pk,
            "".to_string(),
...
pub async fn api_message_job(
    ...
    {
        let files_vec = files.iter().map(|f| HanzoPath::new(f)).collect::<Vec<_>>();
        let job_message = HanzoMessageBuilder::job_message(
            job_id.to_string(),
            content.to_string(),
            files_vec,
            parent.to_string(),
...
pub async fn api_create_job_with_scope(
...
) -> String {
    {
        ...
        let job_creation = HanzoMessageBuilder::job_creation(
            job_scope,
            false,
            subidentity_encryption_sk.clone(),
            clone_signature_secret_key(&subidentity_signature_sk),
...
pub async fn api_get_all_inboxes_from_profile(
    ...
    {
        let inbox_message = HanzoMessageBuilder::get_all_inboxes_for_profile(
            subidentity_encryption_sk.clone(),
            clone_signature_secret_key(&subidentity_signature_sk),
            node_encryption_pk,
            sender_subidentity.to_string(),
