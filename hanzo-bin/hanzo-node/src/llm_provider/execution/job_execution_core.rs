...
use hanzo_message_primitives::{
    schemas::hanzo_name::HanzoName, hanzo_message::hanzo_message_schemas::JobMessage, hanzo_utils::{hanzo_message_builder::HanzoMessageBuilder, signatures::clone_signature_secret_key}
};
...
impl JobManager {
    ...
    async fn handle_error(
        ...
        db.add_message_to_job_inbox(job_id, &hanzo_message, None, ws_manager)
            .await
    ...
    pub async fn process_inference_chain(
        ...
        .map_err(|e| LLMProviderError::HanzoMessageBuilderError(e.to_string()))?;

        hanzo_log(
            HanzoLogOption::JobExecution,
            HanzoLogLevel::Debug,
            format!("process_inference_chain> hanzo_message: {:?}", hanzo_message).as_str(),
        );

        db.add_message_to_job_inbox(&job_message.job_id.clone(), &hanzo_message, None, ws_manager)
            .await?;
    ...
    pub async fn get_image_files_from_message(
        ...
        for filename in &job_message.job_filenames {
                ...
                || filename_lower.ends_with(".gif")
            {
                ...
                match HanzoFileManager::construct_job_file_path(&job_message.job_id, filename, &db) {
                    Ok(file_path) => {
                        // Retrieve the file content
                        match HanzoFileManager::get_file_content(file_path.clone()) {
                            Ok(content) => {
