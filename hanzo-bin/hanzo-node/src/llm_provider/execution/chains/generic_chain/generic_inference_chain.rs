...
impl GenericInferenceChain {
    ...
    fn process_image_files(
        ...
        for file_path in paths {
            if let Some(file_name) = file_path.path.file_name() {
                    ...
                    || filename_lower.ends_with(".gif")
                {
                    ...
                    match HanzoFileManager::get_file_content(file_path.clone()) {
                        Ok(content) => {
        ...
        if let Ok(additional_files) =
            HanzoFileManager::get_absolute_path_for_additional_files(Vec::new(), all_folders)
        {
                ...
                if path.is_file() {
                    if let Some(file_name) = path.file_name() {
                        ...
                        if filename_lower.ends_with(".png")
                            ...
                            || filename_lower.ends_with(".gif")
                        {
                            // Convert path to HanzoPath for consistent handling
                            let hanzo_path = HanzoPath::from_string(path.to_string_lossy().to_string());
                            match HanzoFileManager::get_file_content(hanzo_path.clone()) {
                                Ok(content) => {
                                    let base64_content = base64::engine::general_purpose::STANDARD.encode(&content);
                                    image_files.insert(hanzo_path.relative_path().to_string(), base64_content);
                                }
                                Err(_) => continue,
                            }
                        }
                    ...
                    if let Some(file_name) = path.file_name() {
                            ...
                            || filename_lower.ends_with(".flv")
                        {
                            ...
                            match HanzoFileManager::get_file_content(hanzo_path.clone()) {
                                Ok(content) => {
                                    let base64_content = base64::engine::general_purpose::STANDARD.encode(&content);
                                    video_files.insert(hanzo_path.relative_path().to_string(), base64_content);
                                }
                                Err(_) => continue,
                            }
                        }
                    ...
                    if let Some(file_name) = path.file_name() {
                            ...
                            || filename_lower.ends_with(".aac")
                        {
                            ...
                            match HanzoFileManager::get_file_content(hanzo_path.clone()) {
                                Ok(content) => {
                                    let base64_content = base64::engine::general_purpose::STANDARD.encode(&content);
                                    audio_files.insert(hanzo_path.relative_path().to_string(), base64_content);
                                }
                                Err(_) => continue,
                            }
                        }
    ...
    pub async fn start_chain(
        ...
        */
        ...
        let mut ret_nodes: HanzoFileChunkCollection = HanzoFileChunkCollection {
            chunks: vec![],
            paths: None,
        };
        ...
        if let Ok(file_infos) = HanzoFileManager::get_all_files_and_folders_for_job(&full_job.job_id, &db) {
            for file_info in file_infos {
                let path = HanzoPath::from_string(file_info.path);
        ...
        if tools_allowed && web_search_enabled {
            ...
            let web_search_tool_key = "local:::__official_hanzo:::web_search";
            let download_pages_tool_key = "local:::__official_hanzo:::download_pages";
            let pdf_text_extractor_tool_key = "local:::__official_hanzo:::pdf_text_extractor";
        ...
        loop {
            ...
            if !response.is_function_calls_empty() {
                ...
                for function_call in response.function_calls {
                    ...
                    // Find the HanzoTool that has a tool with the function name
                    let hanzo_tool = tools.iter().find(|tool| {
                    ...
                    if let Some(ref msg_id) = message_hash_id {
                        let trace_info = json!({
                            "tool": hanzo_tool.tool_router_key().to_string_without_version(),
                    ...
                    // TODO: if hanzo_tool is None we need to retry with the LLM (hallucination)
                    let mut function_response = match tool_router
                        .as_ref()
                        .unwrap()
                        .call_function(function_call.clone(), &context, &hanzo_tool, user_profile.clone())
                        .await
                    {
                    ...
                    function_call_with_router_key.tool_router_key =
                        Some(hanzo_tool.tool_router_key().to_string_without_version());
    ...
    pub fn get_additional_files(
        ...
        full_job: &Job,
        job_filenames: Vec<String>,
        merged_fs_files_paths: Vec<HanzoPath>,
        merged_fs_folder_paths: Vec<HanzoPath>,
    ) -> Result<Vec<String>, HanzoFsError> {
        let mut additional_files: Vec<String> = vec![];
        // Get agent/context files
        let f = HanzoFileManager::get_absolute_path_for_additional_files(
            merged_fs_files_paths.clone(),
            merged_fs_folder_paths.clone(),
        )?;
        ...
        if let Ok(folder_path) = folder_path {
            additional_files.extend(HanzoFileManager::get_absolute_paths_with_folder(
                job_filenames.clone(),
