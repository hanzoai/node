...
use hanzo_message_primitives::{
    schemas::hanzo_name::HanzoName, hanzo_message::{
        hanzo_message::{MessageBody, MessageData, HanzoMessage}, hanzo_message_schemas::{JobCreationInfo, JobMessage, MessageSchemaType}
...
pub struct JobManager {
    ...
    pub node_profile_name: HanzoName,
...
impl JobManager {
    ...
    pub async fn new(
        ...
        {
            let db_arc = db.upgrade().ok_or("Failed to upgrade hanzo_db").unwrap();
    ...
    pub async fn process_job_message(
        ...
        let profile = HanzoName::from_hanzo_message_using_recipient_subidentity(&message)?;
                        ...
                        MessageData::Unencrypted(data) => {
                            ...
                            match message_type {
                                MessageSchemaType::JobCreationSchema => {
                                    let agent_name =
                                        HanzoName::from_hanzo_message_using_recipient_subidentity(&message)?;
    ...
    // From JobManager
    /// Checks that the provided HanzoMessage is an unencrypted job message
    pub fn is_job_message(&mut self, message: HanzoMessage) -> bool {
        matches!(
            &message.body,
    ...
    pub async fn process_job_creation(
        ...
        {
            let db_arc = self.db.upgrade().ok_or("Failed to upgrade hanzo_db").unwrap();
            ...
            match db_arc.create_new_job(
                ...
                Ok(_) => (),
                Err(err) => return Err(LLMProviderError::HanzoDB(err)),
            };
            ...
            match db_arc.get_job(&job_id) {
                ...
                Err(err) => Err(LLMProviderError::HanzoDB(err)),
            }
        }
    }
    ...
    async fn update_job_folder_name(
        ...
        let old_folder = db_arc
            ...
            .map_err(|e| LLMProviderError::HanzoDB(e))?;
        ...
        if old_folder.exists() {
            HanzoFileManager::move_folder(old_folder, new_folder, db_arc)
    ...
    pub async fn add_to_job_processing_queue(
        ...
        let sender_subidentity_result = HanzoName::from_hanzo_message_using_sender_subidentity(&message.clone());
