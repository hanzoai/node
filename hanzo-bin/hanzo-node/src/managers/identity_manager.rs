...
use hanzo_crypto_identities::HanzoRegistryError;
use hanzo_message_primitives::schemas::identity::{DeviceIdentity, Identity, StandardIdentity, StandardIdentityType};
...
pub trait IdentityManagerTrait {
    fn find_by_identity_name(&self, full_profile_name: HanzoName) -> Option<&Identity>;
...
impl IdentityManager {
    ...
    pub async fn search_local_identity(&self, full_identity_name: &str) -> Option<Identity> {
        let node_in_question = HanzoName::new(full_identity_name.to_string()).ok()?.extract_node();
...
impl IdentityManagerTrait for IdentityManager {
    ...
    async fn search_identity(&self, full_identity_name: &str) -> Option<Identity> {
        let identity_name = HanzoName::new(full_identity_name.to_string()).ok()?;
    ...
    async fn external_profile_to_global_identity(
        ...
        let full_identity_name = match HanzoName::new(full_profile_name.to_string().clone()) {
            Ok(name) => name,
            Err(_) => {
                hanzo_log(
                    HanzoLogOption::Identity,
...
impl IdentityManager {
    ...
    pub fn verify_message_signature(
        ...
        original_message: &HanzoMessage,
        decrypted_message: &HanzoMessage,
    ) -> Result<(), NodeError> {
