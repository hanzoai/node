...
pub async fn compute_download_store_quest(db: Arc<SqliteManager>) -> Result<bool, String> {
    ...
    let url = std::env::var("HANZO_TOOLS_DIRECTORY_URL")
        .unwrap_or_else(|_| "https://store-api.hanzo.ai/store/defaults".to_string());
...
pub async fn compute_submit_approval_quest(_db: Arc<SqliteManager>, node_name: HanzoName) -> Result<bool, String> {
    let url = format!("https://store-api.hanzo.ai/user/{}/apps", node_name.to_string());
...
pub async fn compute_write_app_reviews_quest(_db: Arc<SqliteManager>, node_name: HanzoName) -> Result<bool, String> {
    let url = format!("https://store-api.hanzo.ai/user/{}/reviews", node_name.to_string());
