...
use hanzo_messages::schemas::{
    ...
    hanzo_preferences::HanzoInternalComms,
    hanzo_tool_offering::{ToolPrice, UsageType, UsageTypeInquiry},
...
impl ToolRouter {
    ...
    pub async fn initialization(&self, embedding_generator: Arc<dyn EmbeddingGenerator>) -> Result<(), ToolError> {
        ...
        let full_identity =
            HanzoName::new(format!("{}/main", node_name)).map_err(|e| ToolError::ExecutionError(e.to_string()))?;
    ...
    pub async fn force_reinstall_all(&self, embedding_generator: Arc<dyn EmbeddingGenerator>) -> Result<(), ToolError> {
        ...
        let node_env = fetch_node_environment();
        let full_identity = HanzoName::new(format!("{}/main", node_env.global_identity_name))
    ...
    async fn import_tools_from_directory(
            ...
            .eq("true")
        {
            ...
            let internal_comms = HanzoInternalComms {
        ...
        let url = env::var("HANZO_TOOLS_DIRECTORY_URL")
            .unwrap_or_else(|_| format!("https://store-api.hanzo.ai/store/defaults"));
                ...
                .map(|(tool_name, tool_url, router_key, new_version, r#type)| {
                    ...
                    async move {
                        if r#type == "Tool" {
                            ...
                            match serde_json::from_value::<HanzoTool>(val["tool"].clone()) {
                                Ok(_tool) => {
                                    println!("Successfully imported tool {} (version: {})", tool_name, new_version);
                                    Ok::<_, ToolError>(("added", tool_name.clone()))
                                }
                                Err(err) => {
                                    eprintln!("Couldn't parse 'tool' field as HanzoTool: {}", err);
    ...
    pub async fn add_network_tool(&self, network_tool: NetworkTool) -> Result<(), ToolError> {
        self.sqlite_manager
            .add_tool(HanzoTool::Network(network_tool, true))
            .await
            .map(|_| ())
            .map_err(|e| ToolError::DatabaseError(e.to_string()))
    }
        ...
        for tool in rust_tools {
            ...
            match self.sqlite_manager.get_tool_header_by_key(&router_key) {
                Err(SqliteManagerError::ToolNotFound(_)) => {
                    added_count += 1;
                    self.sqlite_manager
                        .add_tool(HanzoTool::Rust(rust_tool, true))
                        .await
                        .map_err(|e| ToolError::DatabaseError(e.to_string()))?;
                }
                ...
                Ok(header) => {
                    ...
                    if new_version > current_version {
                        match self.sqlite_manager.get_tool_by_key(&router_key) {
                            Ok(HanzoTool::Rust(existing_rust_tool, is_enabled)) => {
                                ...
                                upgraded_count += 1;
                                self.sqlite_manager
                                    .upgrade_tool(HanzoTool::Rust(upgraded_tool, is_enabled))
                                    .await
                                    .map_err(|e| ToolError::DatabaseError(e.to_string()))?;
                            }
                            Ok(other_variant) => {
    ...
    pub async fn get_tool_by_name(&self, name: &str) -> Result<Option<HanzoTool>, ToolError> {
    ...
    ) -> Result<Option<HanzoTool>, ToolError> {
    ...
    pub async fn vector_search_enabled_tools(
        ...
        query: &str,
        num_of_results: u64,
    ) -> Result<Vec<HanzoToolHeader>, ToolError> {
    ...
    pub async fn call_function(
        ...
        // Add job scope files
        let job_scope =
            HanzoFileManager::get_absolute_path_for_job_scope(&context.db(), &context.full_job().job_id());
        ...
        match hanzo_tool {
            HanzoTool::MCPServer(mcp_server_tool, _is_enabled) => {
            ...
            HanzoTool::Agent(agent_tool, _is_enabled) => {
                let job_callback_manager = context.job_callback_manager();
                let mut job_manager: Option<Arc<Mutex<JobManager>>> = None;
                if let Some(job_callback_manager) = &job_callback_manager {
                    let job_callback_manager = job_callback_manager.lock().await;
