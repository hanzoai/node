...
use hanzo_message_primitives::{
    schemas::{
        ...
        }, prompts::Prompt, hanzo_name::HanzoName
...
pub struct ModelCapabilitiesManager {
    ...
    pub profile: HanzoName,
    pub llm_providers: Vec<SerializedLLMProvider>,
}

impl ModelCapabilitiesManager {
    // Constructor
    pub async fn new(db: Weak<SqliteManager>, profile: HanzoName) -> Self {
    ...
    async fn get_llm_providers(db: &Arc<SqliteManager>, profile: HanzoName) -> Vec<SerializedLLMProvider> {
    ...
    pub fn get_llm_provider_capabilities(model: &LLMProviderInterface) -> Vec<ModelCapability> {
        match model {
            ...
            LLMProviderInterface::HanzoBackend(hanzo_backend) => {
                match hanzo_backend.model_type().to_uppercase().as_str() {
    ...
    pub fn get_llm_provider_cost(model: &LLMProviderInterface) -> ModelCost {
        match model {
            ...
            LLMProviderInterface::HanzoBackend(hanzo_backend) => match hanzo_backend.model_type().as_str() {
    ...
    pub fn get_max_output_tokens(model: &LLMProviderInterface) -> usize {
        match model {
            ...
            LLMProviderInterface::HanzoBackend(hanzo_backend) => {
                // Fill in the appropriate logic for HanzoBackend
                match hanzo_backend.model_type().as_str() {
    ...
    pub fn has_tool_capabilities(model: &LLMProviderInterface, _stream: Option<bool>) -> bool {
        ...
        match model {
            ...
            LLMProviderInterface::HanzoBackend(_) => true,
