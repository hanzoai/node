...
impl Serialize for CoinbaseMPCWallet {
        ...
        S: Serializer,
    {
        ...
        state.serialize_field("lance_db", &"Option<Weak<RwLock<LanceHanzoDb>>>")?;
        state.end()
    }
}
...
impl CoinbaseMPCWallet {
    ...
    pub async fn create_wallet(
        ...
        let mut config = match config {
            Some(cfg) => cfg,
            None => {
                let tool_id = HanzoToolCoinbase::CreateWallet.definition_id();
                let hanzo_tool = sqlite_manager_strong
