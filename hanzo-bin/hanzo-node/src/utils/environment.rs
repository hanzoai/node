...
pub fn fetch_llm_provider_env(global_identity: String) -> Vec<SerializedLLMProvider> {
    ...
    for i in 0..initial_agent_names.len() {
        ...
        let agent = SerializedLLMProvider {
            ...
            full_identity_name: HanzoName::new(format!("{}/main/agent/{}", global_identity, initial_agent_names[i]))
                .unwrap(),
...
pub fn fetch_node_environment() -> NodeEnvironment {
    // Settled by initialize_node once the keys are loaded, and published here.
    let global_identity_name = env::var("GLOBAL_IDENTITY_NAME").unwrap_or_default();
