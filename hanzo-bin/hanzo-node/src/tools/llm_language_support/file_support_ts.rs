pub fn generate_file_support_ts(declaration_only: bool) -> String {
    let function_definitions = vec![
        (
            ...
            "Promise<string[]>",
            vec![],
            r#"const mountPaths = Deno.env.get('HANZO_MOUNT');
    ...
    return mountPaths.split(',').map(path => path.trim()).filter(path => path.length > 0);"#,
        ...
        (
            ...
            "Promise<string[]>",
            vec![],
            r#"const assetPaths = Deno.env.get('HANZO_ASSETS');
    ...
    return assetPaths.split(',').map(path => path.trim()).filter(path => path.length > 0);"#,
        ...
        (
            ...
            "Promise<string>",
            vec![],
            "return Deno.env.get('HANZO_HOME') || \"\";",
            "Home directory path",
        ),
        (
            ...
            "Promise<string>",
            vec![],
            "return Deno.env.get('HANZO_NODE_LOCATION') || \"\";",
            "Hanzo Node URL",
        ),
        (
            "getAccessToken",
    ...
    const oauthConfig: ProviderConfig[] | undefined = JSON.parse(Deno.env.get('HANZO_OAUTH') || '[]');
    if (!oauthConfig) {
