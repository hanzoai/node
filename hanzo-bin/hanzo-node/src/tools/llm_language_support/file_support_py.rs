pub fn generate_file_support_py(declaration_only: bool) -> String {
    let function_definitions = vec![
        (
            ...
            "List[str]",
            vec![],
            r#"mount_paths = os.environ.get('HANZO_MOUNT')
    if not mount_paths:
        return []
    return [path.strip() for path in mount_paths.split(',') if path.strip()]"#,
        ...
        (
            ...
            "List[str]",
            vec![],
            r#"asset_paths = os.environ.get('HANZO_ASSETS')
    if not asset_paths:
        return []
    return [path.strip() for path in asset_paths.split(',') if path.strip()]"#,
        ...
        (
            ...
            "str",
            vec![],
            "return os.environ.get('HANZO_HOME', '')",
            "Home directory path",
        ),
        (
            ...
            "str",
            vec![],
            "return os.environ.get('HANZO_NODE_LOCATION', '')",
            "Hanzo Node URL",
        ),
        (
            "get_access_token",
            ...
            r#"
    oauth_config_str = os.environ.get('HANZO_OAUTH', '{}')
    try:
