    ...
    pub fn new() -> Self {
        Self {
            tool: HanzoToolHeader {
                name: "Hanzo LLM Map Reduce Processor".to_string(),
...
This can be used to process complex requests, text analysis, text matching, text generation, and any other AI LLM task. over long texts."#
                    .to_string(),
                tool_router_key: "local:::__official_hanzo:::hanzo_llm_map_reduce_processor".to_string(),
...
async fn apply_prompt_over_fragment(prompt: String, bearer: String, llm_provider: String, db: Arc<SqliteManager>, 
    node_name: HanzoName, identity_manager: Arc<Mutex<IdentityManager>>, job_manager: Arc<Mutex<JobManager>>,
...
fn get_model_context_size(llm_provider: String, db: Arc<SqliteManager>, node_name: HanzoName) -> Result<usize, ToolError> {
    let hanzo_name = HanzoName::from_node_and_profile_names(node_name.get_node_name_string(), "main".to_string()).map_err(|_| ToolError::ExecutionError("Failed to create hanzo name".to_string()))?;
    let llm_provider = db.get_llm_provider(&llm_provider, &hanzo_name).map_err(|_| ToolError::ExecutionError("Failed to get llm provider".to_string()))?;
