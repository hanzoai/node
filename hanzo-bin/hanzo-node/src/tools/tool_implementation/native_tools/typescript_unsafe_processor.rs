...
impl TypescriptUnsafeProcessorTool {
    pub fn new() -> Self {
        Self {
            tool: HanzoToolHeader {
                name: "Hanzo Typescript Unsafe Processor".to_string(),
                description: "Tool for executing Node.js code. This is unsafe and should be used with extreme caution."
                    .to_string(),
                tool_router_key: "local:::__official_hanzo:::hanzo_typescript_unsafe_processor".to_string(),
    ...
    async fn run_node_code(
        ...
        let random_id = format!("hanzo_stagehand_{}", uuid::Uuid::new_v4());
...
impl ToolExecutor for TypescriptUnsafeProcessorTool {
    async fn execute(
        ...
        let profile_name = HanzoName::from_node_and_profile_names(node_name.node_name, "main".to_string())
        ...
        let temporal_folder = env::temp_dir().join(format!("hanzo_ts_{}", uuid::Uuid::new_v4()));
...
mod tests {
    ...
    async fn test_axios_request() {
        ...
        async function run(config: any, inputs: any) {
            const response = await axios.get('https://hanzo.com');
