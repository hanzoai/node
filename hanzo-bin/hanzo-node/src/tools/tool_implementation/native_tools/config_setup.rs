    ...
    pub fn new() -> Self {
        Self {
            tool: HanzoToolHeader {
                name: "Hanzo Tool Config Updater".to_string(),
...
Example usage:
{
    ...
    "config": {
            ...
            "api_secret": "some-api-secret"
                ...
                tool_router_key: "local:::__official_hanzo:::hanzo_tool_config_updater".to_string(),
                tool_type: "Rust".to_string(),
                formatted_tool_summary_for_ui: "Update HanzoTool configurations".to_string(),
...
",
    ...
    let message = format!(
        "The tool function command was NOT applied. To apply it, call tool 'Hanzo Tool Config Updater' again with the following parameters:
'tool_router_key': {}
'config': {}",
...
mod tests {
    ...
    fn create_deno_tool() -> HanzoTool {
        ...
        let mut initial_tool = HanzoTool::Deno(
            DenoTool {
