...
pub fn get_rust_tools() -> Vec<HanzoToolHeader> {
...
pub async fn get_all_tools(sqlite_manager: Arc<SqliteManager>) -> Vec<HanzoToolHeader> {
...
pub async fn generate_tool_definitions(
    ...
    match language {
        CodeLanguage::Typescript => {
            support_files.insert(
                "hanzo-local-support".to_string(),
                generate_file_support_ts(only_headers),
            );
        }
        CodeLanguage::Python => {
            support_files.insert(
                "hanzo_local_support".to_string(),
    ...
    let all_tools: Vec<HanzoToolHeader> = get_all_tools(sqlite_manager.clone())
        .await
        .into_iter()
        .filter(|tool| {
    ...
    for tool_header in all_tools {
        ...
        let tool_result = match tool_data.clone() {
            HanzoTool::Deno(deno_tool, _) => deno_tool.result,
            HanzoTool::Python(python_tool, _) => python_tool.result,
            HanzoTool::Rust(rust_tool, _) => {
            ...
            HanzoTool::Network(network_tool, _) => {
...
",
    ...
    match language {
        CodeLanguage::Typescript => {
            support_files.insert("hanzo-local-tools".to_string(), output);
        }
        CodeLanguage::Python => {
            support_files.insert("hanzo_local_tools".to_string(), output);
        }
    };
