...
<agent_deno_libraries>
  * Prefer libraries in the following order:
    1. A function provided by './hanzo-local-tools.ts' that resolves correctly the requirement.
...
<agent_libraries>
  * You may use any of the following functions if they are relevant and a good match for the task.
  * Import them with the format: `from hanzo_local_tools import xx`
...
* Prefer libraries in the following order:
  1. A function provided by './hanzo_local_tools.py' that resolves correctly the requirement.
...
pub async fn tool_metadata_implementation_prompt(
    ...
    let oauth_example = if has_oauth {
        r#"[
      {
        ...
        "redirectUrl": "https://secrets.hanzo.com/redirect",
          ...
          "items": {{
            ...
            "properties": {{
              ...
              "redirectUrl": {{
                ...
                "default": "https://secrets.hanzo.com/redirect"
              }},
              "responseType": {{
                "type": "string",
...
<agent_metadata_rules>
  * If the code uses hanzoSqliteQueryExecutor then fill the sqlTables and sqlQueries sections, otherwise these sections are empty.
