...
pub async fn handle_oauth(
    ...
    if let Some(oauth_vec) = oauth {
        for o in oauth_vec {
            ...
            //redirect_uri=https://secrets.hanzo.com/redirect
...
pub async fn execute_mcp_tool_cmd(
    ...
    let preferences_llm_provider_result = match db.get_preference::<String>("default_llm_provider") {
        Ok(Some(provider_id)) => {
            ...
            let profile_name =
                HanzoName::new(format!("{}/main/agent/mcp_default", node_name.get_node_name_string())).unwrap();
...
"#
    ...
    fn test_override_tool_config_no_overrides() {
        ...
        let mut agent = Agent {
            ...
            full_identity_name: HanzoName::new("test.agent".to_string()).unwrap(),
