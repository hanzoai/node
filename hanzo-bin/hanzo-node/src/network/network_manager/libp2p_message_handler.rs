...
/// Message handler that integrates libp2p messages with the existing Hanzo network logic
pub struct HanzoMessageHandler {
...
impl HanzoMessageHandler {
    ...
    pub async fn handle_message_internode(
        ...
        let (encryption_sender_identity, encryption_public_key, actual_sender_name) = if !message.external_metadata.intra_sender.is_empty() {
            ...
            let original_sender_node = HanzoName::new(original_sender_name.clone())
            ...
            let sender_profile_name_string = HanzoName::from_hanzo_message_only_using_sender_node_name(message)
