...
use hanzo_message_primitives::{
    schemas::{
        invoices::{Invoice, InvoiceRequest, InvoiceRequestNetworkError}, hanzo_name::HanzoName
    }, hanzo_message::{
        hanzo_message::{MessageBody, MessageData, HanzoMessage}, hanzo_message_error::HanzoMessageError, hanzo_message_extension::EncryptionStatus, hanzo_message_schemas::MessageSchemaType
    }, hanzo_utils::{
        encryption::clone_static_secret_key, hanzo_logging::{hanzo_log, HanzoLogLevel, HanzoLogOption}, hanzo_message_builder::{HanzoMessageBuilder, HanzoNameString}, signatures::{clone_signature_secret_key, signature_public_key_to_string}
    }
};
...
pub async fn handle_based_on_message_content_and_encryption(
    ...
    channel: Option<ResponseChannel<HanzoMessage>>,
...
pub fn verify_message_signature(sender_signature_pk: VerifyingKey, message: &HanzoMessage) -> io::Result<()> {
...
pub async fn handle_default_encryption(
    ...
    match decrypted_message_result {
        Ok(decrypted_message) => {
            ...
            let hanzo_message = if decrypted_message.is_content_currently_encrypted() {
            ...
            let message = hanzo_message.get_message_content();
            match message {
                Ok(message_content) => {
...
pub async fn handle_network_message_cases(
    ...
    let should_save = matches!(
        ...
        schema_result,
        Err(HanzoMessageError::InvalidMessageSchemaType(err)) if err == "Message data is encrypted"
    );
        ...
        Ok(schema) => {
            match schema {
                MessageSchemaType::InvoiceRequest => {
                    let requester = HanzoName::from_hanzo_message_using_sender_subidentity(&message)?;
                    let receiver = HanzoName::from_hanzo_message_using_recipient_subidentity(&message)?;
                    hanzo_log(
                        HanzoLogOption::Network,
                        HanzoLogLevel::Info,
                        &format!(
                    ...
                    match serde_json::from_str::<Invoice>(&content) {
                        Ok(invoice) => {
                            ...
                            let trace_info = json!({
                                ...
                                "tool_key": invoice.hanzo_offering.tool_key,
...
pub async fn send_ack(
    ...
    let msg = HanzoMessageBuilder::ack_message(
...
pub async fn ping_pong(
    peer: (SocketAddr, HanzoNameString),
    ...
    let msg = HanzoMessageBuilder::ping_pong_message(
        message.to_owned(),
        clone_static_secret_key(&encryption_secret_key),
        signature_secret_key,
        receiver_public_key,
