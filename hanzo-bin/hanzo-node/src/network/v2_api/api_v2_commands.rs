...
use hanzo_http_api::node_api_router::APIUseRegistrationCodeSuccessResponse;
use hanzo_http_api::{
...
use hanzo_message_primitives::{
    ...
    hanzo_message::hanzo_message_schemas::JobCreationInfo,
    hanzo_message::{
...
impl Node {
    ...
    pub fn convert_hanzo_message_to_v2_chat_message(
        hanzo_message: HanzoMessage,
    ) -> Result<V2ChatMessage, NodeError> {
        let internal_metadata = match &hanzo_message.body {
        ...
        let message_data = match &hanzo_message.body {
        ...
        let external_metadata = hanzo_message.external_metadata;
    ...
    pub fn convert_hanzo_messages_to_v2_chat_messages(
        hanzo_messages: Vec<Vec<HanzoMessage>>,
    ...
    pub fn api_v2_create_hanzo_message(
        ...
        let timestamp = HanzoStringTime::generate_time_now();
        ...
        HanzoMessageBuilder::new(node_encryption_sk, node_signing_sk, receiver_public_key)
    ...
    pub async fn v2_api_change_nodes_name(
        ...
        let new_node_name = match HanzoName::from_node_name(new_name) {
            Ok(name) => name,
            Err(_) => {
                let _ = res
                    .send(Err(APIError {
    ...
    pub async fn v2_api_add_agent(
        ...
        let expected_full_identity_name = HanzoName::new(format!(
            "{}/main/agent/{}",
            ...
            if agent_exists {
                ...
                match db.add_agent(agent.clone(), &requester_name) {
                    Ok(_) => {
                        ...
                        if let Err(err) = db.add_tool(hanzo_tool).await {
    ...
    pub async fn v2_api_test_llm_provider(
        ...
        let profile = HanzoName::new(format!("{}/main", name)).unwrap();
        ...
        let provider = SerializedLLMProvider {
            ...
            name: None,
            description: None,
            full_identity_name: HanzoName::new(llm_name).unwrap(),
    ...
    pub async fn v2_api_export_agent(
        ...
        let agent_zip: Result<Vec<u8>, APIError> = generate_agent_zip(db, hanzo_name, node_env, agent_id, true).await;
    ...
    async fn publish_agent(
        ...
        let file_bytes: Vec<u8> =
            generate_agent_zip(db.clone(), hanzo_name.clone(), node_env, agent_id.clone(), true).await?;
    ...
    pub async fn v2_api_check_default_tools_sync(
        ...
        let internal_comms_synced = match db.get_preference::<HanzoInternalComms>("internal_comms") {
        ...
        .await
        {
            Ok(response) => {
                ...
                let galxe_quests_url = std::env::var("GALXE_QUESTS_URL")
                    .unwrap_or_else(|_| "https://api.hanzo.com/galxe/user".to_string());
