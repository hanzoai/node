...
use hanzo_message_primitives::{
    schemas::{
        identity::Identity, inbox_name::InboxName, job::{ForkedJob, JobLike}, job_config::JobConfig, llm_providers::{common_agent_llm_provider::ProviderOrAgent, serialized_llm_provider::SerializedLLMProvider}, hanzo_name::{HanzoName, HanzoSubidentityType}, smart_inbox::{LLMProviderSubset, ProviderType, SmartInbox, V2SmartInbox}
    }, hanzo_message::{
        hanzo_message::{MessageBody, MessageData}, hanzo_message_schemas::{
...
impl Node {
    pub fn convert_smart_inbox_to_v2_smart_inbox(smart_inbox: SmartInbox) -> Result<V2SmartInbox, NodeError> {
        let last_message = match smart_inbox.last_message {
            Some(msg) => Some(Node::convert_hanzo_message_to_v2_chat_message(msg)?),
            None => None,
        };
    ...
    pub async fn v2_create_new_job(
        ...
        match Self::internal_create_new_job(job_manager, db, hanzo_message, main_identity.clone()).await {
            Ok(job_id) => {
    ...
    pub async fn v2_job_message(
        ...
        match Self::internal_job_message(job_manager, hanzo_message.clone(), high_priority.unwrap_or(false)).await {
            Ok(_) => {
                ...
                let scheduled_time = hanzo_message.clone().external_metadata.scheduled_time;
                let message_hash = hanzo_message.calculate_message_hash_for_pagination();
    ...
    pub async fn v2_get_last_messages_from_inbox(
        ...
        let v2_chat_messages = match Self::convert_hanzo_messages_to_v2_chat_messages(messages) {
    ...
    pub async fn v2_update_smart_inbox_name(
        ...
        if let Some(job_id) = inbox.get_job_id() {
            ...
            if old_folder.exists() {
                ...
                if let Err(e) = HanzoFileManager::move_folder(old_folder, new_folder, &db) {
                    let api_error = APIError {
                        code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                        error: "Internal Server Error".to_string(),
    ...
    pub async fn v2_api_retry_message(
        ...
        let sender = match HanzoName::from_hanzo_message_using_sender_subidentity(&message.0) {
            Ok(sender) => sender,
            Err(_) => {
                let sender = match HanzoName::from_hanzo_message_only_using_sender_node_name(&message.0) {
                    Ok(sender) => sender,
                    Err(err) => {
                        let api_error = APIError {
                            code: StatusCode::BAD_REQUEST.as_u16(),
        ...
        let node_name = HanzoName::from_hanzo_message_only_using_sender_node_name(&original_message)
            .unwrap()
            .to_string();

        let sender = match HanzoName::from_node_and_profile_names(node_name, "main".to_string()) {
            Ok(sender) => sender,
            Err(err) => {
                let api_error = APIError {
                    code: StatusCode::BAD_REQUEST.as_u16(),
        ...
        let recipient = match HanzoName::from_hanzo_message_only_using_recipient_node_name(&original_message) {
        ...
        // Process the job message
        match Self::internal_job_message(job_manager, hanzo_message.clone(), false).await {
            Ok(_) => {
                let scheduled_time = hanzo_message.clone().external_metadata.scheduled_time;
                let message_hash = hanzo_message.calculate_message_hash_for_pagination();
            ...
            if let MessageBody::Unencrypted(body) = &message.body {
                if let MessageData::Unencrypted(data) = &body.message_data {
                    if let MessageSchemaType::JobMessageSchema = data.message_content_schema {
                        ...
                        let original_sender = HanzoName::from_hanzo_message_using_sender_subidentity(message)
                            .or_else(|_| HanzoName::from_hanzo_message_only_using_sender_node_name(message))
                            .map_err(|err| APIError {
                                code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                                error: "Internal Server Error".to_string(),
                        ...
                        let original_recipient = HanzoName::from_hanzo_message_using_recipient_subidentity(message)
                            .or_else(|_| HanzoName::from_hanzo_message_only_using_recipient_node_name(message))
                            .map_err(|err| APIError {
                                code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                                error: "Internal Server Error".to_string(),
                        ...
                        let forked_message = Self::api_v2_create_hanzo_message(
                            original_sender,
                            original_recipient,
                            &serde_json::to_string(&job_message).unwrap(),
                            MessageSchemaType::JobMessageSchema,
    ...
    pub async fn v2_add_messages_god_mode(
        ...
        for (index, message) in messages.into_iter().enumerate() {
            if index % 2 == 0 {
                ...
                if let Err(err) = db.add_message_to_job_inbox(&job_id, &hanzo_message, None, None).await {
                    let api_error = APIError {
                        code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                        error: "Internal Server Error".to_string(),
