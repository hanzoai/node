...
impl Node {
    pub async fn v2_api_request_invoice(
        ...
        let network_tool = {
            match db.get_tool_by_key(&tool_key_name) {
                Ok(tool) => match tool {
                    HanzoTool::Network(network_tool, _) => network_tool,
                    _ => {
    ...
    pub async fn v2_api_pay_invoice(
        ...
        let tool_key_name = invoice.hanzo_offering.tool_key.clone();
        let tool = {
            match db.get_tool_by_key(&tool_key_name) {
                Ok(tool) => tool,
                Err(err) => {
        ...
        let required_args = match tool {
            HanzoTool::Network(network_tool, _) => network_tool.input_args,
            _ => {
                let api_error = APIError {
                    code: StatusCode::BAD_REQUEST.as_u16(),
                    error: "Bad Request".to_string(),
