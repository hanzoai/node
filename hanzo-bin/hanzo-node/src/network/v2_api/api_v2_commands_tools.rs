...
use hanzo_message_primitives::{
    schemas::{
        identity::Identity, inbox_name::InboxName, indexable_version::IndexableVersion, job::JobLike, job_config::JobConfig, llm_providers::agent::Agent, hanzo_name::{HanzoName, HanzoSubidentityType}, hanzo_tools::{CodeLanguage, DynamicToolType}, tool_router_key::ToolRouterKey
    }, hanzo_message::hanzo_message_schemas::{CallbackAction, JobCreationInfo, JobMessage, MessageSchemaType}, hanzo_utils::{
        job_scope::MinimalJobScope, hanzo_message_builder::HanzoMessageBuilder, signatures::clone_signature_secret_key
    }
};
use hanzo_sqlite::{errors::SqliteManagerError, SqliteManager};
use hanzo_tools_primitives::tools::{
    agent_tool_wrapper::AgentToolWrapper, deno_tools::DenoTool, error::ToolError, parameters::Parameters, python_tools::PythonTool, hanzo_tool::HanzoToolHeader, hanzo_tool::{HanzoTool, HanzoToolWithAssets}, tool_config::{OAuth, ToolConfig}, tool_output_arg::ToolOutputArg, tool_playground::{ToolPlayground, ToolPlaygroundMetadata}, tool_types::{OperatingSystem, RunnerType, ToolResult}
};
use std::{
...
impl Node {
    /// Searches for Hanzo tools using both vector and full-text search (FTS)
    /// methods.
    ///
    ...
    pub async fn v2_api_search_hanzo_tool(
    ...
    pub async fn v2_api_list_all_hanzo_tools(
        db: Arc<SqliteManager>,
        bearer: String,
        node_name: HanzoName,
        ...
        match db.get_all_tool_headers() {
            Ok(tools) => {
                ...
                let mut tool_groups: HashMap<String, Vec<HanzoToolHeader>> = HashMap::new();
    ...
    pub async fn v2_api_list_all_mcp_hanzo_tools(
        db: Arc<SqliteManager>,
        node_name: HanzoName,
        category: Option<String>,
        tool_router: Option<Arc<ToolRouter>>,
    ...
    pub async fn v2_api_list_all_network_hanzo_tools(
        db: Arc<SqliteManager>,
        bearer: String,
        node_name: HanzoName,
        tool_router: Option<Arc<ToolRouter>>,
    ...
    pub async fn v2_api_set_hanzo_tool(
        db: Arc<SqliteManager>,
        bearer: String,
        tool_router_key: String,
        input_value: Value,
        res: Sender<Result<HanzoTool, APIError>>,
    ) -> Result<(), NodeError> {
        // Validate the bearer token
        if Self::validate_bearer_token(&bearer, db.clone(), &res).await.is_err() {
            return Ok(());
        ...
        let existing_tool = match db.get_tool_by_key(&tool_router_key) {
            ...
            Err(SqliteManagerError::ToolNotFound(_)) => {
                let api_error = APIError {
                    ...
                    message: "Tool not found in LanceHanzoDb".to_string(),
                };
                let _ = res.send(Err(api_error)).await;
                return Ok(());
            }
            Err(err) => {
                let api_error = APIError {
                    code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                    error: "Internal Server Error".to_string(),
                    message: format!("Failed to fetch tool from LanceHanzoDb: {}", err),
                };
                let _ = res.send(Err(api_error)).await;
                return Ok(());
            }
        ...
        // Convert merged_value to HanzoTool
        let merged_tool: HanzoTool = match serde_json::from_value(merged_value) {
            Ok(tool) => tool,
            Err(err) => {
                let api_error = APIError {
                    code: StatusCode::BAD_REQUEST.as_u16(),
                    error: "Bad Request".to_string(),
                    message: format!("Failed to convert merged Value to HanzoTool: {}", err),
                };
                let _ = res.send(Err(api_error)).await;
                return Ok(());
            }
        ...
        // Save the tool to the LanceHanzoDb
        ...
        match save_result {
            ...
            Err(err) => {
                let api_error = APIError {
                    code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                    error: "Internal Server Error".to_string(),
                    message: format!("Failed to add tool to LanceHanzoDb: {}", err),
                };
                let _ = res.send(Err(api_error)).await;
                Ok(())
            }
    ...
    pub async fn v2_api_add_hanzo_tool(
        db: Arc<SqliteManager>,
        bearer: String,
        node_env: NodeEnvironment,
        new_tool_with_assets: HanzoToolWithAssets,
        res: Sender<Result<Value, APIError>>,
    ) -> Result<(), NodeError> {
        // Validate the bearer token
        if Self::validate_bearer_token(&bearer, db.clone(), &res).await.is_err() {
    ...
    pub async fn v2_api_add_network_agent(
        ...
        match save_result {
            Ok(tool) => {
                ...
                let agent_full_identity_name = HanzoName::new(format!(
                    "{}/main/agent/{}",
                ...
                match db.add_agent(agent.clone(), &requester_name) {
                    Ok(_) => {
                        ...
                        let hanzo_agent_tool = HanzoTool::Agent(agent_tool_wrapper, true);

                        // Add agent tool to database
                        if let Err(err) = db.add_tool(hanzo_agent_tool).await {
    ...
    pub async fn v2_api_get_hanzo_tool(
        db: Arc<SqliteManager>,
        bearer: String,
        tool_key: String,
            ...
            Ok(tool) => {
                ...
                if serialize_config {
                    if let Value::Object(ref mut map) = response_value {
                        ...
                        let original_config = tool.get_config(); // Assumes HanzoTool implements GetConfig trait or similar
    ...
    async fn set_playground_tool(
        ...
        let hanzo_tool = match payload.language {
            ...
            CodeLanguage::Python => {
                ...
                HanzoTool::Python(tool, false)
            }
        };

        updated_payload.tool_router_key = Some(hanzo_tool.tool_router_key().to_string_without_version());
        ...
        if let Some(original_tool_key_path) = original_tool_key_path.clone() {
            ...
            delete_old_tool = original_tool_key.to_string_without_version()
                != hanzo_tool.tool_router_key().to_string_without_version();
        }
        ...
        perm_file_path.push(hanzo_tool.tool_router_key().convert_to_path());
        ...
        let version = hanzo_tool.version_indexable()?;
        let version = Some(version);
        let exists = db
            .tool_exists(&hanzo_tool.tool_router_key().to_string_without_version(), version)
            .map_err(|e| APIError {
                code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                error: "Internal Server Error".to_string(),
        ...
        let tool = match exists {
            ...
            true => db.update_tool(hanzo_tool).await,
            // Add the tool to the LanceHanzoDb
            false => db.add_tool(hanzo_tool.clone()).await,
        }
        .map_err(|e| APIError {
            code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
            error: "Internal Server Error".to_string(),
    ...
    pub async fn execute_mcp_tool(
        node_name: HanzoName, // No Bearer token needed because this is an internal tool
        db: Arc<SqliteManager>,
        tool_router_key: String,
        parameters: Map<String, Value>,
        tool_id: String,
    ...
    async fn is_code_generator(
        ...
        let sender = match HanzoName::new(main_identity.get_full_identity_name()) {
            Ok(name) => name,
            Err(_) => return false,
        };
    ...
    pub async fn generate_tool_implementation(
        ...
        let tools = if is_code_generator {
            ...
            let valid_tool_list: Vec<String> = vec![
                ...
                "local:::__official_hanzo:::x_twitter_post",
                "local:::__official_hanzo:::duckduckgo_search",
                "local:::__official_hanzo:::x_twitter_search",
            ]
            .iter()
            .map(|t| t.to_string())
            .collect();
...
",
    ...
    async fn update_job_with_code(
        ...
        let hanzo_message = match Self::api_v2_create_hanzo_message(
            sender,
            recipient,
            ...
            Err(err) => {
                return Err(APIError {
                    code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                    error: "Internal Server Error".to_string(),
                    message: format!("Failed to create Hanzo message: {}", err),
                });
            }
        };

        // Add the Hanzo message to the job inbox
        let add_message_result = db.add_message_to_job_inbox(&job_id, &hanzo_message, None, None).await;

        if let Err(err) = add_message_result {
            return Err(APIError {
                code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                error: "Internal Server Error".to_string(),
                message: format!("Failed to add Hanzo message to job inbox: {}", err),
            });
        }
        ...
        let ai_hanzo_message = HanzoMessageBuilder::job_message_from_llm_provider(
            job_id.to_string(),
            ai_message_content,
            None,
            vec![],
        ...
        let add_ai_message_result = db
            .add_message_to_job_inbox(&job_id, &ai_hanzo_message, None, None)
            .await;
    ...
    pub async fn v2_api_export_tool(
        ...
        match db.get_tool_by_key(&tool_key_path.clone()) {
            Ok(tool) => {
                let file_bytes = generate_tool_zip(db.clone(), hanzo_name.clone(), node_env, tool, true).await;
                match file_bytes {
                    Ok(file_bytes) => {
    ...
    async fn publish_tool(
        ...
        let file_bytes: Vec<u8> = generate_tool_zip(db.clone(), hanzo_name.clone(), node_env, tool, true)
            .await
            .map_err(|e| APIError {
                code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                error: "Internal Server Error".to_string(),
        ...
        let store_url = env::var("HANZO_STORE_URL").unwrap_or("https://store-api.hanzo.com".to_string());
        let response = client
    ...
    pub async fn v2_api_import_tool_url_internal(
        ...
        // Parse the JSON into a HanzoTool
        let tool: HanzoTool = match serde_json::from_slice(&zip_contents.buffer) {
            Ok(tool) => tool,
            Err(err) => {
                return Err(APIError {
                    code: StatusCode::BAD_REQUEST.as_u16(),
    ...
    /// Resolves a Hanzo file protocol URL into actual file bytes.
    ///
    /// The Hanzo file protocol follows the format:
    /// `hanzo://file/{node_name}/{app-id}/{full-path}` This function
    ...
    /// * `hanzo_file_protocol` - The Hanzo file protocol URL to resolve
    ...
    /// ```text
    /// hanzo://file/node123/app-456/path/to/file.txt
    /// ```
    ///
    /// The function will look for this file in:
    /// `{node_storage_path}/tools_storage/app-456/path/to/file.txt`
    pub async fn v2_api_resolve_hanzo_file_protocol(
        bearer: String,
        db: Arc<SqliteManager>,
        hanzo_file_protocol: String,
        ...
        // Parse the hanzo file protocol
        // Format: hanzo://file/{node_name}/{app-id}/{full-path}
        // now we also have hanzo://file/{full-path} (will search on the node storage filesystem)
        let parts: Vec<&str> = hanzo_file_protocol.split('/').collect();
        if parts.len() < 5 || !hanzo_file_protocol.starts_with("hanzo://file/") {
            let api_error = APIError {
                code: StatusCode::BAD_REQUEST.as_u16(),
                error: "Invalid Protocol".to_string(),
                message: "Invalid hanzo file protocol format".to_string(),
            };
            let _ = res.send(Err(api_error)).await;
            return Ok(());
        }
            ...
            Ok(tools) => {
                ...
                for tool in tools {
                    ...
                    match db.get_tool_by_key_and_version(&tool.tool_router_key, Some(version)) {
                        Ok(mut hanzo_tool) => {
                            if hanzo_tool.can_be_enabled() {
                                hanzo_tool.enable();
                                if hanzo_tool.is_enabled() {
                                    let _ = db.update_tool(hanzo_tool.clone()).await.is_ok();
                                }
                            }
                            let activated = hanzo_tool.is_enabled();
    ...
    async fn duplicate_tool(
        ...
        if let HanzoTool::Deno(deno_tool, _enabled) = &mut new_tool {
        ...
        } else if let HanzoTool::Python(python_tool, _enabled) = &mut new_tool {
        ...
        let (new_playground, is_new_playground) = match db.get_tool_playground(&tool_key_path) {
            ...
            Err(_) => {
                ...
                let language = match original_tool {
                    HanzoTool::Deno(_, _) => CodeLanguage::Typescript,
                    HanzoTool::Python(_, _) => CodeLanguage::Python,
    ...
    pub async fn install_tool_from_u8(
        ...
        // Parse the JSON into a HanzoTool
        let tool: HanzoTool = match serde_json::from_slice(&buffer) {
            Ok(tool) => tool,
            Err(err) => {
                return Err(APIError {
                    code: StatusCode::BAD_REQUEST.as_u16(),
    ...
    async fn create_standalone_playground(
        ...
        temp_dir.push(format!("hanzo_playground_{}", uuid::Uuid::new_v4()));
        ...
        let zip_url =
            "https://pub-e508ac8b539c45edb9724730588f74cc.r2.dev/hanzo-tool-boilerplate-feature-user-template.zip";
        ...
        let extracted_dir = temp_dir.join("hanzo-tool-boilerplate-feature-user-template");
        ...
        println!("[Step 4] Updating hanzo-local-tools & hanzo-local-support files");
        // Update hanzo-local-tools & hanzo-local-support files
...
X_HANZO_APP_ID=app-id-{random_uuid}
IDENTITY="{identity}"
LANGUAGE={env_language}
    ...
    pub async fn v2_api_list_all_hanzo_tools_versions(
        db: Arc<SqliteManager>,
        bearer: String,
        res: Sender<Result<Value, APIError>>,
    ) -> Result<(), NodeError> {
    ...
    pub async fn v2_api_get_tools_from_toolset(
        ...
        bearer: String,
        tool_set_key: String,
        res: Sender<Result<Vec<HanzoTool>, APIError>>,
    ) {
        // Validate the bearer token
        if Self::validate_bearer_token(&bearer, db.clone(), &res).await.is_err() {
            return;
    ...
    pub async fn v2_api_get_hanzo_tool_metadata(
        db: Arc<SqliteManager>,
        bearer: String,
        tool_router_key: String,
        res: Sender<Result<Value, APIError>>,
