...
use hanzo_messages::{
    schemas::{
        ...
        hanzo_proxy_builder_info::HanzoProxyBuilderInfo,
        hanzo_tool_offering::{HanzoToolOffering, ToolPrice, UsageTypeInquiry},
        wallet_mixed::{AddressBalanceList, Asset},
    },
    hanzo_message::hanzo_message_schemas::MessageSchemaType,
    hanzo_utils::{
        encryption::clone_static_secret_key, hanzo_message_builder::HanzoMessageBuilder,
...
impl MyAgentOfferingsManager {
    ...
    pub async fn pay_invoice_and_send_receipt(
        ...
        if let Some(db) = self.db.upgrade() {
            ...
            let mut trace_info = json!({
                ...
                "tool_key": updated_invoice.hanzo_offering.tool_key,
                ...
                "tool_price": format!("{:?}", updated_invoice.hanzo_offering.get_price_for_usage(&updated_invoice.usage_type_inquiry)),
    ...
    pub async fn get_balances(&self, node_name: HanzoName) -> Result<AddressBalanceList, AgentOfferingManagerError> {
        let wallet_manager = self.wallet_manager.upgrade().ok_or_else(|| {
            AgentOfferingManagerError::OperationFailed("Failed to upgrade wallet_manager reference".to_string())
        })?;
...
mod tests {
    ...
    fn setup() {
        ...
        let path = Path::new("hanzo_db_tests/");
        let _ = fs::remove_dir_all(path);
    }
    ...
    //     let db = Arc::new(HanzoDB::new("hanzo_db_tests/hanzodb").unwrap());
    ...
    //         LanceHanzoDb::new("lance_db_tests/lancedb", embedding_model.clone(), generator.clone()).await?,
    //     ));
