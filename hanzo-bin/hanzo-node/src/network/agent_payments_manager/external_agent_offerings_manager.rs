...
pub struct ExtAgentOfferingsManager {
    ...
    // pub shared_tools: Arc<DashMap<String, HanzoToolOffering>>, // (streamer_profile:::path, shared_folder)
...
impl ExtAgentOfferingsManager {
    ...
    /// # Arguments
    ///
    /// * `db` - Weak reference to the HanzoDB.
    ...
    pub async fn new(
        ...
        let db_prefix = "hanzo__tool__offering_"; // dont change it
    ...
    /// * `_db` - Weak reference to the HanzoDB.
    ...
    pub async fn update_shareable_tool_requirements(
        &self,
        updated_offering: HanzoToolOffering,
    ...
    pub async fn invoice_requested(
        ...
        let usage_type = match invoice_request.usage_type_inquiry {
            UsageTypeInquiry::PerUse => match hanzo_offering.usage_type {
    ...
    pub async fn confirm_invoice_payment_and_process(
        ...
        let is_free_tool = matches!(
            local_invoice.hanzo_offering.usage_type,
        ...
        let output_opt = if !is_free_tool {
            ...
            let payment_requirements = match &local_invoice.hanzo_offering.usage_type {
            ...
            .unwrap_or_else(|| serde_json::Map::new());
        {
            ...
            // js tool name
            let local_tool_key = local_invoice.hanzo_offering.convert_tool_to_local().map_err(|e| {
