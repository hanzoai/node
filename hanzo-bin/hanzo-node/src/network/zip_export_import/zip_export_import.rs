...
use hanzo_tools::tools::agent_tool_wrapper::AgentToolWrapper;
...
async fn calculate_zip_dependencies(
    ...
    if let Some(agent) = agent_entry_point {
        ...
        let tool: HanzoTool = db.get_tool_by_agent_id(&agent_id).map_err(|e| {
...
async fn get_dependencies_for_zip(
    ...
    tool_dependencies: &HashMap<String, HanzoTool>,
...
fn add_knowledge_embeddings_to_zip(
    ...
    let text_groups: Vec<(String, Vec<f32>)> =
        HanzoFileManager::get_text_groups_with_embeddings(item, &db).map_err(|e| APIError {
            code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
            error: "Internal Server Error".to_string(),
        ...
        if item.clone().exists() {
            ...
            for entry in files {
                    ...
                    .is_file()
                {
                    ...
                    let hanzo_path = HanzoPath::new(&path);
                    ...
                    zip_files.insert(
                        format!("__knowledge/{}", hanzo_path.relative_path().to_string()),
                        file_bytes,
                    );
                    add_knowledge_embeddings_to_zip(db.clone(), hanzo_path, zip_files)?;
                }
            }
        }
    }
...
async fn import_agent_knowledge(
    ...
    for file in files {
        if file.starts_with("__knowledge/") && !file.ends_with("/") {
            ...
            let dest_path = HanzoPath::from_str(&relative_path);

            let file_content = HanzoFileManager::get_file_content(dest_path.clone());
            ...
            if embedding_buffer.is_some() {
                ...
                HanzoFileManager::save_and_process_file_with_embeddings(
                    dest_path,
                    buffer,
                    &db,
                ...
                HanzoFileManager::save_and_process_file(
                    dest_path,
                    buffer,
                    &db,
...
pub async fn import_dependencies_tools(
    ...
    for file_name_str in file_list {
        if file_name_str.starts_with("__tools/") && file_name_str != "__tools/" {
            ...
            let tool: HanzoTool = match serde_json::from_slice(&tool_zip.buffer) {
                Ok(tool) => tool,
                Err(err) => {
                    let api_error = APIError {
                        code: StatusCode::BAD_REQUEST.as_u16(),
...
pub async fn import_mcp_server(
    ...
    if let Ok(tools) = tools {
        for tool in tools {
            ...
            let hanzo_tool = mcp_manager::convert_to_hanzo_tool(
                &tool,
                &mcp_server.name,
            ...
            db.add_tool(hanzo_tool).await.map_err(|e| APIError {
                code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                error: "Database Error".to_string(),
...
pub async fn import_agent(
    ...
    let preferences_llm_provider_result = match db.get_preference::<String>("default_llm_provider") {
        Ok(llm_provider) => match llm_provider {
            ...
            None => Node::hanzo_free_provider_id(),
        },
        Err(_) => {
            return Err(APIError {
                code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
    ...
    let local_full_identity_name = HanzoName::new(format!(
        "{}/main/agent/{}",
    ...
    if install {
        match db.add_agent(agent.clone(), &agent.full_identity_name) {
            Ok(_) => {
                ...
                let hanzo_tool = HanzoTool::Agent(agent_tool_wrapper, true);
                let install_tool = match db.get_tool_by_key(&hanzo_tool.tool_router_key().to_string_without_version())
                {
                    Ok(_) => false,
                    Err(_) => true,
                };
...
pub fn get_tool_from_zip(mut archive: ZipArchive<std::io::Cursor<Vec<u8>>>) -> Result<HanzoTool, APIError> {
    ...
    let tool: HanzoTool = serde_json::from_slice(&buffer).map_err(|e| APIError {
        code: StatusCode::BAD_REQUEST.as_u16(),
        error: "Invalid Tool JSON".to_string(),
...
mod tests {
    ...
    async fn test_tool_dependency_cycles() {
        ...
        tool_a
            .tools
            .push(HanzoTool::Deno(tool_b.clone(), true).tool_router_key());
        tool_b
            .tools
            .push(HanzoTool::Deno(tool_c.clone(), true).tool_router_key());
        tool_c
            .tools
            .push(HanzoTool::Deno(tool_a.clone(), true).tool_router_key());

        // Add tools to database
        db.add_tool_with_vector(
            HanzoTool::Deno(tool_a.clone(), true),
            SqliteManager::generate_vector_for_testing(0.1),
        )
        .unwrap();
        db.add_tool_with_vector(
            HanzoTool::Deno(tool_b.clone(), true),
            SqliteManager::generate_vector_for_testing(0.5),
        )
        .unwrap();
        db.add_tool_with_vector(
            HanzoTool::Deno(tool_c.clone(), true),
        ...
        let result = calculate_zip_dependencies(
            db.clone(),
            profile.clone(),
            Some(HanzoTool::Deno(tool_a.clone(), true)),
            None,
            None,
            &mut agent_dependencies,
            &mut tool_dependencies,
    ...
    async fn test_tool_dependency_cycles_agent() {
        ...
        let agent_tool_wrapper = HanzoTool::Agent(
    ...
    async fn test_agent_tool_dependencies() {
        ...
        // Add tools to database
        let tool_a_wrapper = HanzoTool::Deno(tool_a.clone(), true);
        let tool_b_wrapper = HanzoTool::Deno(tool_b.clone(), true);
