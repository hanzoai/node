...
pub struct HanzoNetworkBehaviour {
...
pub enum NetworkEvent {
    ...
    SendDirectMessage { peer_id: PeerId, message: HanzoMessage },
    /// A response to be sent to a specific peer using request-response
    SendResponse {
        channel: ResponseChannel<HanzoMessage>,
...
pub struct LibP2PManager {
    ...
    message_handler: Arc<HanzoMessageHandler>,
...
use crate::network::network_manager::libp2p_message_handler::HanzoMessageHandler;
...
impl LibP2PManager {
    ...
    pub async fn new(
        ...
        message_handler: HanzoMessageHandler,
        ...
        let mut swarm = libp2p::SwarmBuilder::with_existing_identity(local_key)
            ...
            .with_behaviour(move |keypair, relay_behaviour| HanzoNetworkBehaviour {
                ...
                identify: identify::Behaviour::new(
                    identify::Config::new("/hanzo/1.0.0".to_string(), keypair.public())
                        .with_agent_version(format!("hanzo-node-{}", node_name))
                ...
                request_response: request_response::json::Behaviour::new(
                    std::iter::once((
                        libp2p::StreamProtocol::new("/hanzo/message/1.0.0"),
                        request_response::ProtocolSupport::Full,
                    )),
    ...
    pub async fn send_direct_message_to_peer(
        ...
        if !is_connected {
            ...
            hanzo_log(
                ...
                HanzoLogLevel::Debug,
                &format!(
                    "Message queued for peer {}, queue size: {}",
                    peer_id,
    ...
    async fn handle_swarm_event(
        &mut self,
        event: SwarmEvent<HanzoNetworkBehaviourEvent>,
    ) -> Result<(), Box<dyn std::error::Error>> {
        match event {
            ...
            SwarmEvent::Behaviour(HanzoNetworkBehaviourEvent::RelayClient(relay_event)) => {
                ...
                match relay_event {
                    RelayClientEvent::ReservationReqAccepted {
                        ...
                        hanzo_log(
                            ...
                            HanzoLogLevel::Info,
                            &format!(
                                "🎉 Relay reservation ACCEPTED by {} (renewal: {}, limit: {:?})",
                    ...
                    RelayClientEvent::OutboundCircuitEstablished { relay_peer_id, limit } => {
                        hanzo_log(
                            ...
                            HanzoLogLevel::Debug,
                            &format!(
                                "🔄 Outbound circuit established through {} (limit: {:?})",
                                relay_peer_id, limit
                            ),
                        );
                    }
            ...
            SwarmEvent::Behaviour(HanzoNetworkBehaviourEvent::Dcutr(dcutr_event)) => {
            ...
            SwarmEvent::Behaviour(HanzoNetworkBehaviourEvent::Ping(ping_event)) => {
                let ping::Event {
                    peer,
                    connection: _,
                    result,
            ...
            SwarmEvent::Behaviour(HanzoNetworkBehaviourEvent::Identify(identify::Event::Received {
                peer_id,
                info,
                ..
            })) => {
                ...
                let supports_relay = info
                    .protocols
                    .iter()
                    .any(|protocol| protocol.to_string().contains("/libp2p/circuit/relay/"));
                ...
                if supports_relay {
                    ...
                    hanzo_log(
                        ...
                        HanzoLogLevel::Debug,
                        &format!(
                            "Peer {} identified as direct connection peer (no relay support)",
                            peer_id
                        ),
                    );

                    // Check if this peer supports our Hanzo message protocol
                    let supports_hanzo = info
                        .protocols
                        .iter()
                        .any(|protocol| protocol.to_string().contains("/hanzo/message/"));
                    ...
                    if supports_hanzo {
                        hanzo_log(
                            ...
                            HanzoLogLevel::Info,
                            &format!(
                                "✅ Peer {} supports Hanzo message protocol - ready for communication",
                                peer_id
                            ),
                        );
                    } else {
                        hanzo_log(
                            HanzoLogOption::Network,
                            HanzoLogLevel::Debug,
                            &format!("⚠️  Peer {} does not support Hanzo message protocol", peer_id),
                        );
                    }
                }
                    ...
                    request_response::Event::Message { peer, message, .. } => {
                        match message {
                            ...
                            request_response::Message::Response {
                                ...
                                let _ = self
                                    .message_handler
                                    .handle_message_internode(peer, &response, None, Some(self.event_sender.clone()))
                                    .await;
                        ...
                        match error {
                            request_response::OutboundFailure::DialFailure => {
                                hanzo_log(
                                    ...
                                    HanzoLogLevel::Debug,
                                    &format!(
                                        "Dial failure to peer {} - will retry when connection is established",
                                        peer
                                    ),
                                );

                                if let Some(mut queued_message) = self.pending_outbound_requests.remove(&request_id) {
                                    if !self.message_queue.iter().any(|m| {
                                        m.peer_id == queued_message.peer_id && m.message == queued_message.message
                                    }) {
            ...
            SwarmEvent::ConnectionEstablished { peer_id, endpoint, .. } => {
                ...
                if Self::is_circuit_address(&endpoint.get_remote_address()) {
                    hanzo_log(
                        ...
                        HanzoLogLevel::Debug,
                        &format!(
                            "Peer {} connected via relay - will attempt DCUtR upgrade after identification",
                            peer_id
                        ),
                    );
                }
        ...
        if let Some(ref relay_addr) = self.relay_address.clone() {
            ...
            if !self.is_connected_to_relay && !actually_connected_to_relay {
                ...
                hanzo_log(
                    ...
                    HanzoLogLevel::Info,
                    &format!(
                        "🔄 No existing relay connection found - attempting reconnection (attempt {}) to: {}",
    ...
    fn calculate_backoff_duration(&self) -> Duration {
        ...
        let delay_seconds = std::cmp::min(
            base_delay * (2_u32.saturating_pow(self.reconnection_attempts)),
            max_delay,
        );
    ...
    fn mark_relay_connected(&mut self, peer_id: PeerId) {
        ...
        if !self.is_connected_to_relay || self.relay_peer_id != Some(peer_id) {
            ...
            hanzo_log(
                ...
                HanzoLogLevel::Info,
                &format!(
                    "✅ Relay connection established with {} - reconnection state reset",
                    peer_id
                ),
            );
        }
            ...
            if relay_peer == peer_id {
                ...
                if !self.swarm.is_connected(&peer_id) {
                    ...
                    hanzo_log(
                        ...
                        HanzoLogLevel::Info,
                        &format!(
                            "📡 One connection to relay {} closed, but other connections remain active",
                            peer_id
                        ),
                    );
                }
    ...
    pub async fn connect_to_discovered_peer(&mut self, identity: &str) -> Result<(), Box<dyn std::error::Error>> {
        if let Some((_peer_id, circuit_addr)) = self.discovered_peers.get(identity).cloned() {
            hanzo_log(
                ...
                HanzoLogLevel::Info,
                &format!(
                    "🔗 Connecting to discovered peer {} via circuit: {}",
                    identity, circuit_addr
                ),
            );
            ...
            hanzo_log(
                ...
                HanzoLogLevel::Info,
                &format!(
                    "✅ Initiated connection to peer {} - waiting for connection establishment",
                    identity
                ),
            );
        } else {
        ...
        for (index, queued_message) in self.message_queue.iter().enumerate() {
            ...
            if queued_message.retry_count >= self.max_retry_attempts {
                hanzo_log(
                    ...
                    HanzoLogLevel::Error,
                    &format!(
                        "Message to peer {} exceeded max retry attempts ({}), dropping",
        ...
        for &index in messages_to_retry.iter().rev() {
            if let Some(mut queued_message) = self.message_queue.remove(index) {
                hanzo_log(
                    ...
                    HanzoLogLevel::Debug,
                    &format!(
                        "Retrying message to peer {} (attempt {})",
                        queued_message.peer_id,
                ...
                self.pending_outbound_requests
                    .insert(_request_id, queued_message.clone());
...
pub fn verifying_key_to_peer_id(
