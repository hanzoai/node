...
impl LibP2PManager {
    ...
    async fn handle_swarm_event(
        ...
        match event {
            ...
            SwarmEvent::Behaviour(HanzoNetworkBehaviourEvent::Identify(identify::Event::Received { peer_id, info, .. })) => {
                hanzo_log(
                    HanzoLogOption::Network,
                    HanzoLogLevel::Debug,
                ...
                if supports_relay {
                    ...
                    let supports_hanzo = info.protocols.iter().any(|protocol| {
                        protocol.to_string().contains("/hanzo/message/")
                    });
                    ...
                    if supports_hanzo {
                        hanzo_log(
                            HanzoLogOption::Network,
                            HanzoLogLevel::Info,
                            &format!("✅ Peer {} supports Hanzo message protocol - ready for communication", peer_id),
                        );
                    } else {
                        hanzo_log(
                            HanzoLogOption::Network,
