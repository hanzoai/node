...
use hanzo_message_primitives::{
    schemas::{
        inbox_name::InboxName, llm_providers::serialized_llm_provider::{LLMProviderInterface, Ollama, SerializedLLMProvider}, hanzo_name::HanzoName
    }, hanzo_message::hanzo_message::HanzoMessage, hanzo_utils::{
        hanzo_logging::{hanzo_log, HanzoLogLevel, HanzoLogOption}, signatures::clone_signature_secret_key
    }
};
...
impl Node {
    ...
    pub async fn internal_get_all_inboxes_for_profile(
        identity_manager: Arc<Mutex<IdentityManager>>,
        db: Arc<SqliteManager>,
        full_profile_name: HanzoName,
    ) -> Vec<String> {
        // Obtain the IdentityManager and HanzoDB locks
    ...
    pub async fn internal_update_smart_inbox_name(
        ...
        if let Some(job_id) = job_id {
            ...
            if old_folder.exists() {
                use hanzo_fs::hanzo_file_manager::HanzoFileManager;
                HanzoFileManager::move_folder(old_folder, new_folder, &db)
    ...
    pub async fn internal_create_new_job(
        ...
        match job_manager.process_job_message(hanzo_message, false).await {
            Ok(job_id) => {
    ...
    pub async fn internal_get_llm_providers_for_profile(
        ...
        let profile_name = match HanzoName::from_node_and_profile_names(node_name, profile) {
    ...
    pub async fn internal_job_message(
        ...
        let mut job_manager = job_manager.lock().await;
        match job_manager.process_job_message(hanzo_message, high_priority).await {
            Ok(_) => Ok(()),
                    ...
                    .await
                {
                    Ok(_) => {
                        ...
                        if !has_job_inbox && welcome_message {
                            let hanzo_folder_fs = HanzoPath::from_string("/My Files (Private)".to_string());

                            let job_scope = MinimalJobScope {
                                vector_fs_items: vec![],
                                vector_fs_folders: vec![hanzo_folder_fs],
                            ...
                            db.unsafe_update_smart_inbox_name(
                                ...
                                "Welcome to Hanzo! Brief onboarding here.",
                            )?;

                            {
                                ...
                                let upload_file_path = HanzoPath::from_string(
                                ...
                                let _ = HanzoFileManager::write_file_to_fs(upload_file_path, upload_file_content);

                                let welcome_message = include_str!("../../../../../files/hanzo_welcome.md");
                                ...
                                db.add_message_to_job_inbox(&job_id, &hanzo_message, None, None)
                                    .await
                                    .map_err(|e| NodeError {
