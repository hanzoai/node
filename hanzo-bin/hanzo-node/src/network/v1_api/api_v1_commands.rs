...
use hanzo_message_primitives::{
    schemas::{
        ...
        hanzo_message::{MessageBody, MessageData, HanzoMessage}, hanzo_message_schemas::{
        ...
        }, hanzo_logging::{hanzo_log, HanzoLogLevel, HanzoLogOption}, signatures::{clone_signature_secret_key, signature_public_key_to_string, string_to_signature_public_key}
    }
};
...
impl Node {
    pub async fn validate_message(
        ...
        node_name: &HanzoName,
        potentially_encrypted_msg: HanzoMessage,
        schema_type: Option<MessageSchemaType>,
    ) -> Result<(HanzoMessage, Identity), APIError> {
    ...
    pub async fn has_inbox_access(
        ...
        let sender_hanzo_name = HanzoName::new(sender_subidentity.get_full_identity_name())
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;

        let has_creation_permission = inbox_name.has_creation_access(sender_hanzo_name);
    ...
    where
        F: FnOnce(Vec<Vec<HanzoMessage>>) -> T,
    {
    ...
    pub async fn api_get_last_messages_from_inbox(
        ...
        node_name: HanzoName,
        potentially_encrypted_msg: HanzoMessage,
        res: Sender<Result<Vec<HanzoMessage>, APIError>>,
    ) -> Result<(), NodeError> {
    ...
    pub async fn api_get_last_messages_from_inbox_with_branches(
        ...
        node_name: HanzoName,
        potentially_encrypted_msg: HanzoMessage,
        res: Sender<Result<Vec<Vec<HanzoMessage>>, APIError>>,
    ) -> Result<(), NodeError> {
        Self::process_last_messages_from_inbox(
            encryption_secret_key,
            db,
    ...
    pub async fn handle_registration_code_usage(
        ...
        let _profile_list: Vec<HanzoName> = match db.get_all_profiles(node_name.clone()) {
            ...
            Ok(success) => {
                match identity_type {
                    IdentityType::Profile | IdentityType::Global => {
                        ...
                        let full_identity_name_result = HanzoName::from_node_and_profile_names(
                    ...
                    IdentityType::Device => {
                        ...
                        // Check if the profile exists in the identity_manager
                        {
                            ...
                            let profile_identity_name = HanzoName::from_node_and_profile_names(
                                node_name.get_node_name_string(),
                                profile_name.clone(),
                            )
                            .unwrap();
                        ...
                        // let full_identity_name = format!("{}/{}", self.node_profile_name.clone(),
                        // profile_name.clone());
                        let full_identity_name = HanzoName::from_node_and_profile_names_and_type_and_name(
                            node_name.get_node_name_string(),
                            profile_name,
    ...
    pub async fn api_get_all_inboxes_for_profile(
        ...
        let profile_requested: HanzoName = if HanzoName::validate_name(&profile_requested_str).is_ok() {
            HanzoName::new(profile_requested_str.clone()).map_err(|err| err.to_string())?
        } else {
            HanzoName::from_node_and_profile_names(node_name.get_node_name_string(), profile_requested_str.clone())
    ...
    pub async fn api_job_message(
        ...
        let sender_hanzo_name = sender_identity.get_full_identity_name();
        let sender_node_name = match HanzoName::new(sender_hanzo_name) {
    ...
    pub async fn api_available_llm_providers(
        ...
        let profile = HanzoName::from_hanzo_message_using_sender_subidentity(&msg.clone())?
    ...
    pub async fn api_add_ollama_models(
        ...
        // Convert HanzoName to StandardIdentity if necessary and check if it's a Profile type with admin privileges
    ...
    pub async fn api_remove_agent(
        ...
        let profile = match HanzoName::new(profile) {
            Ok(profile) => profile,
            Err(err) => {
                let api_error = APIError {
                    code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
    ...
    #[allow(clippy::too_many_arguments)]
    pub async fn api_search_hanzo_tool(
        ...
        let (search_query, requester_name) = match Self::validate_and_extract_payload::<String>(
            ...
            encryption_secret_key,
            potentially_encrypted_msg,
            MessageSchemaType::SearchHanzoTool,
        )
        .await
        {
            Ok(data) => data,
    ...
    pub async fn api_list_all_hanzo_tools(
        ...
        let requester_name = match Self::validate_and_extract_payload::<String>(
            ...
            encryption_secret_key,
            potentially_encrypted_msg,
            MessageSchemaType::ListAllHanzoTools,
        )
        .await
        {
    ...
    pub async fn api_set_hanzo_tool(
        sqlite_manager: Arc<SqliteManager>,
        node_name: HanzoName,
        identity_manager: Arc<Mutex<IdentityManager>>,
        encryption_secret_key: EncryptionStaticKey,
        ...
        let (input_value, requester_name) = match Self::validate_and_extract_payload::<serde_json::Value>(
            ...
            encryption_secret_key,
            potentially_encrypted_msg,
            MessageSchemaType::SetHanzoTool,
        )
        .await
        {
            Ok(data) => data,
    ...
    pub async fn api_get_hanzo_tool(
        sqlite_manager: Arc<SqliteManager>,
        node_name: HanzoName,
        identity_manager: Arc<Mutex<IdentityManager>>,
        encryption_secret_key: EncryptionStaticKey,
        ...
        let (tool_key, requester_name) = match Self::validate_and_extract_payload::<String>(
            ...
            encryption_secret_key,
            potentially_encrypted_msg,
            MessageSchemaType::GetHanzoTool,
        )
        .await
        {
            Ok(data) => data,
        ...
        // Fetch the tool from the LanceHanzoDb
    ...
    pub async fn api_change_nodes_name(
        ...
        let new_node_name = match HanzoName::from_node_name(new_node_name) {
            Ok(name) => name,
            Err(_) => {
                let _ = res
                    .send(Err(APIError {
    ...
    pub async fn api_handle_send_onionized_message(
        ...
        //
        let recipient_node_name = HanzoName::from_hanzo_message_only_using_recipient_node_name(&msg.clone())
            .unwrap()
            .get_node_name_string();

        let sender_node_name = HanzoName::from_hanzo_message_only_using_sender_node_name(&msg.clone())
            .unwrap()
            .get_node_name_string();
        ...
        let recipient_node_name_string = HanzoName::from_hanzo_message_only_using_recipient_node_name(&msg.clone())
            .unwrap()
            .to_string();
