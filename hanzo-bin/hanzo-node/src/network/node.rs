...
use super::network_manager::libp2p_message_handler::HanzoMessageHandler;
...
impl Node {
    ...
    pub async fn new(
        ...
        let node_name = HanzoName::new(node_name).unwrap();
        {
        ...
        let proxy_connection_info: Option<ProxyConnectionInfo> = proxy_identity.and_then(|proxy_identity| {
            HanzoName::new(proxy_identity.clone())
                .inspect_err(|_| {
                    hanzo_log(
                        HanzoLogOption::Node,
                        HanzoLogLevel::Error,
    ...
    pub async fn start(&mut self) -> Result<(), NodeError> {
        ...
        // Initialize LibP2P networking
        {
            ...
            let message_handler = HanzoMessageHandler::new(
            ...
            match LibP2PManager::new(self.node_name.to_string(), self.identity_secret_key.clone(), listen_port, message_handler, relay_address).await {
                ...
                Err(e) => {
                    hanzo_log( // we'll show an error but not stop node initialization.
                        HanzoLogOption::Network,
                        HanzoLogLevel::Error,
    ...
    // Static function to get the address from a HanzoName identity
    ...
    pub async fn save_to_db(
        ...
        if is_body_encrypted {
            ...
            if am_i_sender {
                counterpart_identity = HanzoName::from_hanzo_message_only_using_recipient_node_name(message)
                    .unwrap()
                    .to_string();
            } else {
                counterpart_identity = HanzoName::from_hanzo_message_only_using_sender_node_name(message)
                    .unwrap()
                    .to_string();
            }
    ...
    pub fn hanzo_free_provider_id() -> String {
        "hanzo_free_trial".to_string()
    }
}
