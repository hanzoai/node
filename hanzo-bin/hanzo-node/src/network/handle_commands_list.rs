...
impl Node {
    pub async fn handle_command(&self, command: NodeCommand) {
        match command {
            ...
            //     hanzo_log(HanzoLogOption::Node, HanzoLogLevel::Info, "Shutdown command received. Stopping the
            // node.");     // self.db = Arc::new(Mutex::new(HanzoDB::new("PLACEHOLDER").expect("Failed to
            ...
            NodeCommand::V2ApiPublishAgent { bearer, agent_id, res } => {
                ...
                let hanzo_name = self.node_name.clone();
                let identity_manager_clone = self.identity_manager.clone();
            ...
            NodeCommand::V2ApiExportAgent { bearer, agent_id, res } => {
                ...
                let hanzo_name = self.node_name.clone();
                tokio::spawn(async move {
                    let _ = Node::v2_api_export_agent(db_clone, bearer, hanzo_name, node_env, agent_id, res).await;
                });
            }
            ...
            NodeCommand::V2ApiHanzoBackendGetQuota {
                bearer,
                model_type,
                res,
            } => {
                let db_clone = Arc::clone(&self.db);
                tokio::spawn(async move {
                    let _ = Node::v2_api_check_hanzo_backend_quota(db_clone, model_type, bearer, res).await;
                });
            }
            ...
            NodeCommand::V2ApiHealthCheck { res } => {
                ...
                tokio::spawn(async move {
                    let _ = Node::v2_api_health_check(
                        db_clone,
                        public_https_certificate_clone,
                        is_migration_in_progress_clone,
                        res,
                    )
                    .await;
                });
            ...
            NodeCommand::V2ApiTriggerEmbeddingMigration { bearer, payload, res } => {
                ...
                let is_migration_in_progress_clone = Arc::clone(&self.is_migration_in_progress);
                tokio::spawn(async move {
                    let _ = Node::v2_api_trigger_embedding_migration(
                        db_clone,
                        embedding_generator_clone,
                        default_embedding_model_clone,
                        is_migration_in_progress_clone,
                        bearer,
                        payload,
                        res,
            ...
            NodeCommand::V2ApiGetMigrationStatus { bearer, res } => {
                ...
                tokio::spawn(async move {
                    let _ =
                        Node::v2_api_get_migration_status(db_clone, is_migration_in_progress_clone, bearer, res).await;
                });
            }
            ...
            NodeCommand::V2ApiListAllHanzoTools { bearer, category, res } => {
                let db_clone = Arc::clone(&self.db);
                let tool_router_clone = self.tool_router.clone();
                let node_name_clone = self.node_name.clone();
                tokio::spawn(async move {
                    let _ = Node::v2_api_list_all_hanzo_tools(
                        db_clone,
                        bearer,
                        node_name_clone,
                        category,
            ...
            NodeCommand::V2ApiListAllMcpHanzoTools { category, res } => {
                let db_clone = Arc::clone(&self.db);
                let tool_router_clone = self.tool_router.clone();
                let node_name_clone = self.node_name.clone();
                tokio::spawn(async move {
                    let _ = Node::v2_api_list_all_mcp_hanzo_tools(
                        db_clone,
                        node_name_clone,
                        category,
                        tool_router_clone,
            ...
            NodeCommand::V2ApiListAllNetworkHanzoTools { bearer, res } => {
                let db_clone = Arc::clone(&self.db);
                let tool_router_clone = self.tool_router.clone();
                let node_name_clone = self.node_name.clone();
                tokio::spawn(async move {
                    let _ = Node::v2_api_list_all_network_hanzo_tools(
                        db_clone,
                        bearer,
                        node_name_clone,
                        tool_router_clone,
            ...
            NodeCommand::V2ApiListAllHanzoToolsVersions { bearer, res } => {
                let db_clone = Arc::clone(&self.db);
                tokio::spawn(async move {
                    let _ = Node::v2_api_list_all_hanzo_tools_versions(db_clone, bearer, res).await;
                });
            }
            NodeCommand::V2ApiGetHanzoToolMetadata {
                bearer,
                tool_router_key,
                res,
            } => {
                let db_clone = Arc::clone(&self.db);
                tokio::spawn(async move {
                    let _ = Node::v2_api_get_hanzo_tool_metadata(db_clone, bearer, tool_router_key, res).await;
                });
            }
            ...
            NodeCommand::V2ApiSetHanzoTool {
                ...
                let db_clone = Arc::clone(&self.db);
                tokio::spawn(async move {
                    let _ = Node::v2_api_set_hanzo_tool(db_clone, bearer, tool_key, payload, res).await;
                });
            }
            NodeCommand::V2ApiAddHanzoTool {
                bearer,
                hanzo_tool,
                res,
            } => {
                ...
                let node_env = fetch_node_environment();
                tokio::spawn(async move {
                    let _ = Node::v2_api_add_hanzo_tool(db_clone, bearer, node_env, hanzo_tool, res).await;
                });
            }
            ...
            NodeCommand::V2ApiExportTool {
                ...
                tokio::spawn(async move {
                    let _ =
                        Node::v2_api_export_tool(db_clone, bearer, hanzo_name, node_env, tool_key_path, res).await;
                });
            }
            ...
            NodeCommand::V2ApiResolveHanzoFileProtocol {
                bearer,
                hanzo_file_protocol,
                res,
            } => {
                ...
                let node_name = self.node_name.clone();
                tokio::spawn(async move {
                    let _ = Node::v2_api_resolve_hanzo_file_protocol(
                        bearer,
                        db_clone,
                        hanzo_file_protocol,
                        node_storage_path,
            ...
            NodeCommand::V2ApiSearchHanzoTool {
                bearer,
                query,
                agent_or_llm,
                res,
                ...
                let db_clone = Arc::clone(&self.db);
                tokio::spawn(async move {
                    let _ = Node::v2_api_search_hanzo_tool(db_clone, bearer, query, agent_or_llm, res).await;
                });
            }
