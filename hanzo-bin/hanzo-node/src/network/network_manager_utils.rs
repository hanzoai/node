...
use hanzo_message_primitives::{
    schemas::{identity::StandardIdentity, hanzo_proxy_builder_info::HanzoProxyBuilderInfo}, hanzo_message::hanzo_message::HanzoMessage, hanzo_utils::hanzo_logging::{hanzo_log, HanzoLogLevel, HanzoLogOption}
};
...
pub async fn get_proxy_builder_info_static(
    ...
    if let Some(proxy_connection) = proxy_connection_info.as_ref() {
        ...
        match identity_manager
            ...
            .await
        {
            Ok(proxy_identity) => Some(HanzoProxyBuilderInfo {
...
pub async fn send_message_to_peer(
    ...
    let receiver_socket_addr = receiver_identity.addr.ok_or_else(|| {
        AgentOfferingManagerError::OperationFailed(
            format!(
                "Hanzo ID doesn't have a valid socket address: {}",
                receiver_identity.full_identity_name.extract_node()
            )
            .to_string(),
        )
