...
use hanzo_tools_primitives::tools::{
    mcp_server_tool::MCPServerTool, parameters::{Parameters, Property}, hanzo_tool::HanzoTool, tool_config::ToolConfig, tool_output_arg::ToolOutputArg, tool_types::ToolResult
};
use toml::Table;

/// Converts an rmcp Tool to a HanzoTool::MCPServer
pub fn convert_to_hanzo_tool(
    tool: &Tool,
    server_name: &str,
    server_id: &str,
    ...
    // Return the HanzoTool
    HanzoTool::MCPServer(mcp_tool, true)
}
...
pub mod tests_mcp_manager {
    ...
    fn test_convert_to_hanzo_tool() {
        ...
        let hanzo_tool = convert_to_hanzo_tool(
            tool,
            server_name,
            server_id,
            server_command_hash,
        ...
        if let HanzoTool::MCPServer(mcp_tool, enabled) = hanzo_tool {
            ...
            panic!("Expected HanzoTool::MCPServer variant");
        }
    }
