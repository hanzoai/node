...
get_current_version() {
    local tool_name=$1
    local tool_dir="$SCRIPT_DIR/../pre-install"
...
mkdir -p "$SCRIPT_DIR/../pre-install"

# Clean up old tool files
echo "Cleaning up old tool files..."
if [ -d "$SCRIPT_DIR/../pre-install" ]; then
    find "$SCRIPT_DIR/../pre-install" -name "*.zip" | while read -r file; do
    ...
    if [ ! -z "$tool" ]; then
        ...
        if [ ! -z "$name" ] && [ ! -z "$version" ] && [ ! -z "$file_url" ]; then
            ...
            if [ "$should_download" = true ]; then
                ...
                # Download the file
                curl -L "$file_url" -o "$SCRIPT_DIR/../pre-install/${sanitized_name}_${sanitized_version}.zip"
